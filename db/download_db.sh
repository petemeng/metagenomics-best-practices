...
Usage:
  ...
  DB_ROOT=/absolute/path db/download_db.sh eukdetect2-install
  DB_ROOT=/absolute/path db/download_db.sh eukdetect2-verify
  DB_ROOT=/absolute/path db/download_db.sh metabolism-install
  DB_ROOT=/absolute/path db/download_db.sh metabolism-verify
  DB_ROOT=/absolute/path db/download_db.sh gem-install
  DB_ROOT=/absolute/path db/download_db.sh gem-verify
  DB_ROOT=/absolute/path db/download_db.sh community-install
  DB_ROOT=/absolute/path db/download_db.sh community-verify
...
Set DB_MANIFEST=data/small/54-virus-database-manifest.tsv to retrieve the
geNomad v1.9, CheckV v1.5, and VirSorter2 v0.4 archives. VirSorter2 publishes
its combined viral HMM as three byte-range parts; verify all three, concatenate
them in numeric order, and then decompress the resulting gzip stream.
Article 58 uses `eukdetect2-install` to retrieve and verify the 14 files in the
EukDetect2 2026-03-16 marker database (Zenodo DOI 10.5281/zenodo.19056625).
The per-file byte counts and publisher MD5 values are locked in
data/small/58-eukdetect2-database-manifest.tsv.
Article 59 uses `metabolism-install` to retrieve pinned DRAM 1.5.0 and
METABOLIC-G v4.0 source commits plus KOfam 2026-06-01, dbCAN 5.2.9 and MEROPS
resources. Twelve assets are checked against
data/small/59-metabolism-database-manifest.tsv before extraction is accepted.
Article 60 uses `gem-install` to retrieve the checksum-locked gapseq sequence
database v1.5 (Zenodo 20446806), extract its six publisher-checked nested
archives, and audit the CarveMe 1.6.6 universes and annotation database from
the locked environment. Set CARVEME_ENV when the environment is not at the
default path shown in env/carveme.yml.
Article 61 uses `community-install` to retrieve the 264,422,408-byte AGORA
2.01 / RefSeq 216 species-level MICOM database, its fixed source manifest, and
the fixed-commit Western-diet medium. All three assets are byte- and
SHA-256-gated; the Zenodo model archive is also checked against its publisher
MD5. Set DB_ROOT to keep these resources outside the repository if desired.
...
PY
  ...
  eukdetect2-install)
    [[ "$#" -eq 1 ]] || die "eukdetect2-install takes no positional arguments"
    [[ -n "${DB_ROOT:-}" ]] || DB_ROOT="${project_root}/db/microbial-eukaryotes-cache/eukdetect2-v2026-03-16"
    bash "${project_root}/scripts/download_article58_eukdetect2_db.sh" \
      "${project_root}" "${DB_ROOT}" download
    ;;

  eukdetect2-verify)
    [[ "$#" -eq 1 ]] || die "eukdetect2-verify takes no positional arguments"
    [[ -n "${DB_ROOT:-}" ]] || DB_ROOT="${project_root}/db/microbial-eukaryotes-cache/eukdetect2-v2026-03-16"
    bash "${project_root}/scripts/download_article58_eukdetect2_db.sh" \
      "${project_root}" "${DB_ROOT}" verify
    ;;

  metabolism-install)
    [[ "$#" -eq 1 ]] || die "metabolism-install takes no positional arguments"
    [[ -n "${DB_ROOT:-}" ]] || DB_ROOT="${project_root}/db/metabolism-cache"
    python3 "${project_root}/scripts/download_article59_metabolism_db.py" \
      --project-root "${project_root}" --cache-dir "${DB_ROOT}" download
    ;;

  metabolism-verify)
    [[ "$#" -eq 1 ]] || die "metabolism-verify takes no positional arguments"
    [[ -n "${DB_ROOT:-}" ]] || DB_ROOT="${project_root}/db/metabolism-cache"
    python3 "${project_root}/scripts/download_article59_metabolism_db.py" \
      --project-root "${project_root}" --cache-dir "${DB_ROOT}" verify
    ;;

  gem-install)
    [[ "$#" -eq 1 ]] || die "gem-install takes no positional arguments"
    [[ -n "${DB_ROOT:-}" ]] || DB_ROOT="${project_root}/db/gem-cache"
    CARVEME_ENV="${CARVEME_ENV:-/workspace/miniconda3/envs/metagenome-carveme-2026.08}"
    python3 "${project_root}/scripts/download_article60_gem_db.py" \
      --project-root "${project_root}" --cache-dir "${DB_ROOT}" \
      --carveme-env "${CARVEME_ENV}" download
    ;;

  gem-verify)
    [[ "$#" -eq 1 ]] || die "gem-verify takes no positional arguments"
    [[ -n "${DB_ROOT:-}" ]] || DB_ROOT="${project_root}/db/gem-cache"
    CARVEME_ENV="${CARVEME_ENV:-/workspace/miniconda3/envs/metagenome-carveme-2026.08}"
    python3 "${project_root}/scripts/download_article60_gem_db.py" \
      --project-root "${project_root}" --cache-dir "${DB_ROOT}" \
      --carveme-env "${CARVEME_ENV}" verify
    ;;

  community-install)
    [[ "$#" -eq 1 ]] || die "community-install takes no positional arguments"
    [[ -n "${DB_ROOT:-}" ]] || DB_ROOT="${project_root}/db/community-cache"
    python3 "${project_root}/scripts/download_article61_community_db.py" \
      --project-root "${project_root}" --cache-dir "${DB_ROOT}" download
    ;;

  community-verify)
    [[ "$#" -eq 1 ]] || die "community-verify takes no positional arguments"
    [[ -n "${DB_ROOT:-}" ]] || DB_ROOT="${project_root}/db/community-cache"
    python3 "${project_root}/scripts/download_article61_community_db.py" \
      --project-root "${project_root}" --cache-dir "${DB_ROOT}" verify
    ;;
