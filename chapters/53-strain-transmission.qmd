---
author: "Peter"
title: "菌株传播与共享：母婴传播、FMT 定植与证据等级"
...
::: {.callout-tip title="共享信号需要怎样的背景比较"}

共享信号需要独立的时间和来源证据才能谈传播；共同暴露或共同来源仍可能解释相似性。

:::

## 菌株共享与传播，差在哪些证据？ {#sec-target}
...
本例中，配对母婴与未配对样本的共享物种数中位数都是 2；在 7 个可比较家庭中，也只有 3 个婴儿与配对母亲的 Jaccard 相似度最高。单看“检出了相同物种”会忽略这些物种在背景人群中的常见程度。匹配关系必须与实际背景比较一起展示，不能只保留支持家庭共享的样本。

即使更精细的序列证据支持共享谱系，采样时点、共同环境与未采集的接触者仍可能影响解释。相似性不能单独确定方向或接触链；多次比较也不会自动变成独立事件。这里能判断的是哪些观测与家庭共享相容，不能据此还原一次具体传播过程。

## 下载、按成员校验并登记来源 {#sec-code}

### 同种只说明两个样本进入了同一个粗坐标 {#sec-theory}
...
### 检查 1：species presence threshold 改变结果，但不挽救 source identification {#sec-audit}
