---
title: "去宿主、复杂度评估与重复 reads 的识别和处理"
author: "Peter"
date: last-modified
format:
  html:
...
::: {.callout-tip title="去重改变的是测量，而不只是文件大小"}

检查人源残留的同时检查微生物误删；重复序列可能来自真实高丰度成员，不自动等于 PCR 重复。

:::

## 去宿主与去重复，为什么不能一起默认全删？ {#sec-target}
...
在高度不均匀的微生物群落中，多个相同 reads 可能是同一高丰度成员的真实采样，也可能来自技术扩增；仅凭序列相同无法区分。默认去重会对高丰度和小基因组成员产生不同影响，因此主丰度分析不应无条件删除这些记录。若有 UMI 或可靠定位信息，也要说明它们支持的重复定义。

去宿主则需要同时考虑人源残留和微生物误删。本例两个控制验证了给定输入与参考下的表现，不覆盖所有人群变异或所有微生物。某一控制的高删除率不能被写成零隐私风险，微生物 mock 的高保留率也不是全部自然群落的保真保证。

## 下载本例数据与分析代码 {#sec-reader-inputs}

[数据与代码清单](https://github.com/petemeng/metagenomics-best-practices/blob/e219a2cd01609cc208a89e291cbd363ed7f2fbd8/examples/14/files.tsv)列出了本例的输入表、分析脚本和环境文件（约 1.2 MiB）。本清单不含原始 FASTQ 或大型数据库；是否需要它们取决于下文的分析起点。清单中的已计算结果仅供核对；读取结果表不等于重新运行统计模型或上游工具。

在新建文件夹中运行下面的 R 代码，下载完成后即可按正文中的相对路径读取文件。需要核验文件版本时，可使用[带完整性检查的下载脚本](https://raw.githubusercontent.com/petemeng/metagenomics-best-practices/62d27927776d6fd61479767fc219d590fb8259d2/examples/14/download.R)。

```{r}
#| label: reader-input-download-14
#| eval: false
options(timeout = 600)
...
### 先区分论文全量 benchmark 与确定性前缀控制 {#sec-audit}
