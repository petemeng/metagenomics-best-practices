---
author: "Peter"
title: "基因组尺度代谢模型：gapseq / CarveMe"
...
::: {.callout-tip title="一个可行的代谢解，能说明多少生物学"}

培养基边界和补缺反应都会影响预测；模型可行不等于真实生长或通量已被测量。

:::

## 代谢模型预测生长，依赖哪些假设？ {#sec-target}
...
FBA 给出的是满足当前反应、边界和目标函数的一组可行解，不是对真实细胞每条反应通量的直接测量。多个不同解可能达到相同目标值；求解成功与预测在实验中成立也不是同一件事。因此，本例的 168 次计算检查主要用于定位模型假设的影响，不是 168 次独立生物学验证。

gap filling 可以恢复数学可行性，却也可能加入缺乏当前基因组支持的反应。比较完整与截短 MAG 时，应同时看保留的证据和补入的假设，不能只比较是否得到正的生长目标值。模型越依赖补全，结论越需要写成条件性预测。

## 下载本例数据与分析代码 {#sec-reader-inputs}

[数据与代码清单](https://github.com/petemeng/metagenomics-best-practices/blob/e219a2cd01609cc208a89e291cbd363ed7f2fbd8/examples/60/files.tsv)列出了本例的输入表、分析脚本和环境文件（约 16.9 MiB）。本清单不含原始 FASTQ 或大型数据库；是否需要它们取决于下文的分析起点。清单中的已计算结果仅供核对；读取结果表不等于重新运行统计模型或上游工具。

在新建文件夹中运行下面的 R 代码，下载完成后即可按正文中的相对路径读取文件。需要核验文件版本时，可使用[带完整性检查的下载脚本](https://raw.githubusercontent.com/petemeng/metagenomics-best-practices/62d27927776d6fd61479767fc219d590fb8259d2/examples/60/download.R)。

```{r}
#| label: reader-input-download-60
#| eval: false
options(timeout = 600)
...
## 准备 8 个 MAG、4 个 sensitivity inputs 和共享 proteins {#sec-code}

### 从 MAG 到 GEM 至少经过五层对象 {#sec-theory}
