---
author: "Peter"
title: "分箱器：MetaBAT2、SemiBin2、VAMB 与 TaxVAMB"
...
::: {.callout-tip title="候选 bin 更多，为什么最终可用基因组未必更多"}

在同一输入和质量标准下比较分箱结果；产出数量不能替代完整度、污染和嵌合检查。

:::

## 多个分箱器，怎样比较才公平？ {#sec-target}
...
### 候选 bin 更多，为什么最终可用基因组未必更多？

一个基因组可以被拆成多个不完整 bin，也可以与别的成员混成一个较大的 bin。五个工具产出数量不同，需要在共同序列范围与同一质量定义下比较，而不能把最大数量直接解释为最大回收。真值只在选择之后评价，避免把真实项目无法知道的答案用作调参依据。

taxonomy-assisted 分箱还依赖参考覆盖：较熟悉的成员可能获益，未知谱系则未必具有相同信息。完整度、污染与嵌合度反映不同错误模式，应保留逐 bin 结果。即便通过最小质量门槛，也不意味着已经达到完整 MIMAG 高质量标准。

## 下载本例数据与分析代码 {#sec-reader-inputs}

[数据与代码清单](https://github.com/petemeng/metagenomics-best-practices/blob/e219a2cd01609cc208a89e291cbd363ed7f2fbd8/examples/42/files.tsv)列出了本例的输入表、分析脚本和环境文件（约 121.1 MiB）。本清单不含原始 FASTQ 或大型数据库；是否需要它们取决于下文的分析起点。清单中的已计算结果仅供核对；读取结果表不等于重新运行统计模型或上游工具。

在新建文件夹中运行下面的 R 代码，下载完成后即可按正文中的相对路径读取文件。需要核验文件版本时，可使用[带完整性检查的下载脚本](https://raw.githubusercontent.com/petemeng/metagenomics-best-practices/62d27927776d6fd61479767fc219d590fb8259d2/examples/42/download.R)。

```{r}
#| label: reader-input-download-42
#| eval: false
options(timeout = 600)
...
## 准备 common-coordinate inputs {#sec-code}
...
### 从“每个工具默认跑一次”升级为 matched benchmark {#sec-audit}
...
## 加入 post-hoc known-truth audit

### CheckM2/GUNC 与 known truth 是互补记录表
...
### Post-hoc truth proxy 不等于 MIMAG
...
## 检查 single 与 multisample design
...
### Composition、coverage 与 taxonomy 的信息并不等价 {#sec-theory}
...
把五个分箱分支放在同一 contig 与多样本深度坐标上，以 CheckM2/GUNC 做 truth-blind 选择，再用已知基因组事后审计。
