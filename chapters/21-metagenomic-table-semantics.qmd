---
title: "宏基因组表格到底是什么：counts、coverage、CPM、relative abundance 与 genome equivalents"
author: "Peter"
date: last-modified
draft: false
format:
...
::: {.callout-tip title="数字像整数，也未必适合计数模型"}

先区分观测计数、模型估计、长度校正值和相对丰度；单位没确认之前，不做求和、稀释或差异检验。

:::

## 同样叫 abundance，表里的数字究竟是什么？ {#sec-target}
...
[数据与代码清单](https://github.com/petemeng/metagenomics-best-practices/blob/e219a2cd01609cc208a89e291cbd363ed7f2fbd8/examples/21/files.tsv)列出了本例的输入表、分析脚本和环境文件（约 13.1 MiB）。本清单不含原始 FASTQ 或大型数据库；是否需要它们取决于下文的分析起点。清单中的已计算结果仅供核对；读取结果表不等于重新运行统计模型或上游工具。

在新建文件夹中运行下面的 R 代码，下载完成后即可按正文中的相对路径读取文件。需要核验文件版本时，可使用[带完整性检查的下载脚本](https://raw.githubusercontent.com/petemeng/metagenomics-best-practices/62d27927776d6fd61479767fc219d590fb8259d2/examples/21/download.R)。

```{r}
#| label: reader-input-download-21
#| eval: false
options(timeout = 600)
...
### Measurement object：每个数在数什么 {#sec-theory}
