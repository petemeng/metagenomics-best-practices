---
title: "安装 R、curatedMetagenomicData 与关键包生态"
author: "Peter"
date: last-modified
format:
  html:
...
::: {.callout-tip title="对象读取成功之后，先确认 assay 的单位"}

先确认包版本和资源标识，再检查对象维度、单位与样本信息；缓存可用不等于来源已经记录完整。

:::

## 从可用的 R 环境取回一个真实宏基因组对象 {#sec-target}
...
### 对象读取成功之后，先确认 assay 的单位

298 行 × 24 列只说明对象维度，不能说明每一行是什么分类层级、每个值是 counts 还是百分比。相对丰度不能重新命名为整数计数再用于 count likelihood；同一受试者的多个样本也不能只因列名不同就作为独立重复。这里 24 个样本来自 15 位受试者，设计信息必须随 assay 一起保留。

ExperimentHub 标识、资源标题和处理版本共同说明数据身份。缓存使复现更快，但重新取回一个更新资源后，仍需检查样本、分类层级及单位是否改变。对象转换时按明确的 ID 对齐，而不是依赖行列数量相等；保留原始资源与转换规则，才能判断后续差异来自科学方法还是输入变化。

## 下载本例数据与分析代码 {#sec-reader-inputs}

[数据与代码清单](https://github.com/petemeng/metagenomics-best-practices/blob/e219a2cd01609cc208a89e291cbd363ed7f2fbd8/examples/12/files.tsv)列出了本例的输入表、分析脚本和环境文件（约 0.3 MiB）。本清单不含原始 FASTQ 或大型数据库；是否需要它们取决于下文的分析起点。清单中的已计算结果仅供核对；读取结果表不等于重新运行统计模型或上游工具。

在新建文件夹中运行下面的 R 代码，下载完成后即可按正文中的相对路径读取文件。需要核验文件版本时，可使用[带完整性检查的下载脚本](https://raw.githubusercontent.com/petemeng/metagenomics-best-practices/62d27927776d6fd61479767fc219d590fb8259d2/examples/12/download.R)。

```{r}
#| label: reader-input-download-12
#| eval: false
options(timeout = 600)
...
## 先结果核对运行时和精确包版本 {#sec-code}

### R 与 Bioconductor 是配套 release {#sec-theory}
...
## 记录 R、Bioconductor 与资源版本 {#sec-methods}
