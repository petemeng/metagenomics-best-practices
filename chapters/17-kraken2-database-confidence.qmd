---
title: "数据库与置信阈值：Kraken2 库选择、confidence 与假阳性控制"
author: "Peter"
date: last-modified
draft: false
format:
...
::: {.callout-tip title="提高阈值应同时报告丢掉了什么"}

阈值的影响取决于数据库内容；用已知组成对照比较召回和非预期调用，不把 confidence 当概率。

:::

## 提高 confidence，哪些检出会一起消失？ {#sec-target}
...
[数据与代码清单](https://github.com/petemeng/metagenomics-best-practices/blob/e219a2cd01609cc208a89e291cbd363ed7f2fbd8/examples/17/files.tsv)列出了本例的输入表、分析脚本和环境文件（约 1.3 MiB）。本清单不含原始 FASTQ 或大型数据库；是否需要它们取决于下文的分析起点。清单中的已计算结果仅供核对；读取结果表不等于重新运行统计模型或上游工具。

在新建文件夹中运行下面的 R 代码，下载完成后即可按正文中的相对路径读取文件。需要核验文件版本时，可使用[带完整性检查的下载脚本](https://raw.githubusercontent.com/petemeng/metagenomics-best-practices/62d27927776d6fd61479767fc219d590fb8259d2/examples/17/download.R)。

```{r}
#| label: reader-input-download-17
#| eval: false
options(timeout = 600)
