...
::: {.callout-tip title="高 mapping rate 不等于每条 read 都可唯一计数"}

区分多重比对分配、基因长度校正与样本内归一化；功能一对多映射不能解释为新增 reads。

:::

## 从 reads 到基因丰度，分母在哪里变化？ {#sec-target}
...
### 高 mapping rate 不等于每条 read 都可唯一计数

一条 read 可以比对到多个近缘基因；把所有命中都累加会制造新的计数质量。应先固定多重比对和配对规则，再区分 mapped、通过过滤与实际 assigned 的数量。基因长度校正和样本总量归一化是后续的不同步骤，不应混进原始 assignment 账本。

本例 reads 已参与目录构建，因此高回比率不是外部泛化性能。功能一对多映射又会改变求和规则：多个标签不代表多条新增 reads。比较样本时应保留未分配与未注释部分，否则功能图的分母可能随样本而变化。

## 下载本例数据与分析代码 {#sec-reader-inputs}

[数据与代码清单](https://github.com/petemeng/metagenomics-best-practices/blob/e219a2cd01609cc208a89e291cbd363ed7f2fbd8/examples/35/files.tsv)列出了本例的输入表、分析脚本和环境文件（约 8.6 MiB）。本清单不含原始 FASTQ 或大型数据库；是否需要它们取决于下文的分析起点。清单中的已计算结果仅供核对；读取结果表不等于重新运行统计模型或上游工具。

在新建文件夹中运行下面的 R 代码，下载完成后即可按正文中的相对路径读取文件。需要核验文件版本时，可使用[带完整性检查的下载脚本](https://raw.githubusercontent.com/petemeng/metagenomics-best-practices/62d27927776d6fd61479767fc219d590fb8259d2/examples/35/download.R)。

```{r}
#| label: reader-input-download-35
#| eval: false
options(timeout = 600)
...
### 检查 1：10,000-read mapping 适合作为 catalog coverage 诊断，不适合作正式 abundance {#sec-audit}
...
### 先定义计数单位：read、fragment 还是 alignment {#sec-theory}
