---
title: "Kraken2 + Bracken：k-mer 分类与丰度重估"
author: "Peter"
date: last-modified
draft: false
format:
...
::: {.callout-tip title="重分配增加物种计数，但没有增加原始序列证据"}

先读清 Kraken2 的层级计数，再解释 Bracken 重分配；不要把模型估计当成原始物种 read counts。

:::

## Kraken2 分类与 Bracken 丰度，回答的是两个问题 {#sec-target}
...
[数据与代码清单](https://github.com/petemeng/metagenomics-best-practices/blob/e219a2cd01609cc208a89e291cbd363ed7f2fbd8/examples/16/files.tsv)列出了本例的输入表、分析脚本和环境文件（约 0.3 MiB）。本清单不含原始 FASTQ 或大型数据库；是否需要它们取决于下文的分析起点。清单中的已计算结果仅供核对；读取结果表不等于重新运行统计模型或上游工具。

在新建文件夹中运行下面的 R 代码，下载完成后即可按正文中的相对路径读取文件。需要核验文件版本时，可使用[带完整性检查的下载脚本](https://raw.githubusercontent.com/petemeng/metagenomics-best-practices/62d27927776d6fd61479767fc219d590fb8259d2/examples/16/download.R)。

```{r}
#| label: reader-input-download-16
#| eval: false
options(timeout = 600)
...
### 把 2017/2019 算法放进 2026 数据库约定 {#sec-audit}
...
### Kraken2 查询的是 minimizer evidence {#sec-theory}
...
## 独立重画 redistribution 与 parameter sensitivity
