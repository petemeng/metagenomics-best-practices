...
::: {.callout-tip title="宿主比例下降后，分母与样本代表性都可能改变"}

把宿主去除率和微生物保留率同时报告；建库前耗竭与计算去宿主解决的是不同问题。

:::

## 降低宿主比例之后，微生物信号保住了吗？ {#sec-target}
...
[数据与代码清单](https://github.com/petemeng/metagenomics-best-practices/blob/e219a2cd01609cc208a89e291cbd363ed7f2fbd8/examples/06/files.tsv)列出了本例的输入表、分析脚本和环境文件（约 0.0 MiB）。本清单不含原始 FASTQ 或大型数据库；是否需要它们取决于下文的分析起点。清单中的已计算结果仅供核对；读取结果表不等于重新运行统计模型或上游工具。

在新建文件夹中运行下面的 R 代码，下载完成后即可按正文中的相对路径读取文件。需要核验文件版本时，可使用[带完整性检查的下载脚本](https://raw.githubusercontent.com/petemeng/metagenomics-best-practices/62d27927776d6fd61479767fc219d590fb8259d2/examples/06/download.R)。

```{r}
#| label: reader-input-download-06
#| eval: false
options(timeout = 600)
...
## 从论文原值重算 read budget 与剂量权衡 {#sec-code}

### 低生物量要看微生物负荷与宿主背景，不只看总 DNA {#sec-theory}
...
### 忠实复现 Marotz 2018 的计算定义 {#sec-audit}
