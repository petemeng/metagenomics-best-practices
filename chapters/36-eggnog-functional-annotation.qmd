---
title: "Gene-centric 功能注释：eggNOG-mapper 与功能暗物质"
author: "Peter"
date: last-modified
draft: false
format:
...
::: {.callout-tip title="未注释既可能来自新颖性，也可能来自序列不完整"}

分别报告可注释比例与已注释功能组成；同源注释是功能推断，不是活性测量。

:::

## 未注释基因，为什么不能从分母里消失？ {#sec-target}
...
### 未注释既可能来自新颖性，也可能来自序列不完整

没有 seed ortholog 与有 ortholog 但没有具体 KO/GO 是不同层次的缺失。短 ORF 或组装错误也可能降低命中，所以不能把全部 no-hit 都称为新功能。这里按 ORF 完整度分层，有助于区分部分技术原因与参考覆盖不足。

gene fraction 给每条目录记录等权，read-weighted fraction 则强调样本中被更多序列支持的部分。两者不同不是计算错误，而是在回答目录覆盖和样本组成两个问题。注释转移仍是条件性功能推断；需要更具体功能结论时，应增加相应证据，而不是把宽泛同源描述改写成确定底物或活性。

## 下载本例数据与分析代码 {#sec-reader-inputs}

[数据与代码清单](https://github.com/petemeng/metagenomics-best-practices/blob/e219a2cd01609cc208a89e291cbd363ed7f2fbd8/examples/36/files.tsv)列出了本例的输入表、分析脚本和环境文件（约 27.7 MiB）。本清单不含原始 FASTQ 或大型数据库；是否需要它们取决于下文的分析起点。清单中的已计算结果仅供核对；读取结果表不等于重新运行统计模型或上游工具。

在新建文件夹中运行下面的 R 代码，下载完成后即可按正文中的相对路径读取文件。需要核验文件版本时，可使用[带完整性检查的下载脚本](https://raw.githubusercontent.com/petemeng/metagenomics-best-practices/62d27927776d6fd61479767fc219d590fb8259d2/examples/36/download.R)。

```{r}
#| label: reader-input-download-36
#| eval: false
options(timeout = 600)
...
### 检查 6：PFAM transfer 不等于 de novo domain confirmation
...
### Seed ortholog 是入口，不是终点 {#sec-theory}
