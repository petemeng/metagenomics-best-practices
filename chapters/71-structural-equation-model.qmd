...
20 名健康对照没有抗生素暴露者，说明这部分人群缺乏目标比较所需的重叠。回归仍可能输出一个系数，但其中一部分依赖模型外推，而非同类对象之间的实际对照。外部队列也没有暴露者时，可以检查某些测量关系，却不能完成整条暴露路径的外部验证。

间接路径区间跨 0 是估计层面的不确定性；横断面时序不明与缺乏重叠是识别层面的限制。增加样本或情景模拟中的检验功效，并不会自动消除后两者。要缩小因果主张，首先应改善时间信息与比较设计，而不是不断修改路径直到拟合指标变好。
...
在新建文件夹中运行下面的 R 代码，下载完成后即可按正文中的相对路径读取文件。需要核验文件版本时，可使用[带完整性检查的下载脚本](https://raw.githubusercontent.com/petemeng/metagenomics-best-practices/62d27927776d6fd61479767fc219d590fb8259d2/examples/71/download.R)。
...
::: {.callout-tip title="没有可比的暴露者，模型是在拟合什么"}
