...
::: {.callout-tip title="功能注释高度一致，是否意味着功能已被验证"}

同源注释、模块完整度与代谢潜力要分开；不完整 MAG 的缺失不能直接写成功能缺失。

:::

## 通路相关基因存在，是否足以支持代谢功能？ {#sec-target}
...
[数据与代码清单](https://github.com/petemeng/metagenomics-best-practices/blob/e219a2cd01609cc208a89e291cbd363ed7f2fbd8/examples/59/files.tsv)列出了本例的输入表、分析脚本和环境文件（约 23.0 MiB）。本清单不含原始 FASTQ 或大型数据库；是否需要它们取决于下文的分析起点。清单中的已计算结果仅供核对；读取结果表不等于重新运行统计模型或上游工具。

在新建文件夹中运行下面的 R 代码，下载完成后即可按正文中的相对路径读取文件。需要核验文件版本时，可使用[带完整性检查的下载脚本](https://raw.githubusercontent.com/petemeng/metagenomics-best-practices/62d27927776d6fd61479767fc219d590fb8259d2/examples/59/download.R)。

```{r}
#| label: reader-input-download-59
#| eval: false
options(timeout = 600)
...
## 准备 MAG、数据库链接与确定性 sensitivity inputs {#sec-code}

### Profile HMM 命中首先是序列证据 {#sec-theory}
...
### 同一 KOfam snapshot 不等于同一 workflow {#sec-audit}
