---
title: "公共宏基因组资源：MGnify、GEM、GTDB 与环境 MAG catalog"
...
::: {.callout-tip title="公共目录中没有记录，为什么不等于自然界没有"}

先区分原始归档、标准化分析、基因组目录和分类骨架；明确独立单位与版本后再合并数据。

:::

## 公共资源里，应该先下载哪一张表？ {#sec-target}
...
### 公共目录中没有记录，为什么不等于自然界没有？

一个 MAG 进入目录，需要依次经过采样、测序、组装、分箱和质量筛选。每一层都可能与环境、丰度和基因组特征有关。因此，目录中的地区分布或类群比例既反映自然差异，也反映公开数据与恢复流程的选择；不能直接当作全球流行率或完整生态分布。

本例 10,450 个输入 metagenomes 中，7,304 个在最终表里贡献了 MAG。按 MAG 行数计数会进一步让回收较多的样本获得更大权重。比较环境时应先确定要描述的是输入研究、独立样本还是恢复基因组，并保留零恢复样本的可能性；分类更新也应保留原标签，避免把更名误判成生物学变化。

## 下载本例数据与分析代码 {#sec-reader-inputs}

[数据与代码清单](https://github.com/petemeng/metagenomics-best-practices/blob/e219a2cd01609cc208a89e291cbd363ed7f2fbd8/examples/73/files.tsv)列出了本例的输入表、分析脚本和环境文件（约 13.6 MiB）。本清单不含原始 FASTQ 或大型数据库；是否需要它们取决于下文的分析起点。清单中的已计算结果仅供核对；读取结果表不等于重新运行统计模型或上游工具。

在新建文件夹中运行下面的 R 代码，下载完成后即可按正文中的相对路径读取文件。需要核验文件版本时，可使用[带完整性检查的下载脚本](https://raw.githubusercontent.com/petemeng/metagenomics-best-practices/62d27927776d6fd61479767fc219d590fb8259d2/examples/73/download.R)。

```{r}
#| label: reader-input-download-73
#| eval: false
options(timeout = 600)
...
## 先画 accession crosswalk，不要先下载 FASTQ {#sec-code}

### Archive 层保存身份，analysis 层保存一次版本化计算 {#sec-theory}
...
## 用样本级 protocol 建 shotgun-DNA eligibility
...
### Study title 只用于 discovery，eligibility 落到每一行
...
## metadata completeness 用 missingness 表，不用默认值
...
### 忠实保留 resource-native identifier，再建立自己的 cohort key {#sec-audit}
...
### Pipeline-version harmonization 先于 batch correction
...
## 复用 MGnify output 时同时锁 analysis 与 pipeline lineage
...
## 用 GEM metadata 回答覆盖问题，不为画图下载 FASTA
...
### MAG row、species OTU 与 catalogue representative 是三个单位
...
## 先选 biome catalogue，再选择 taxonomy 文件或完整 reference

### GTDB cluster 是 taxonomy backbone，不是生态丰度
...
### Catalogue 的 absence 是“未恢复/未入库”，不是环境缺失
...
### Taxonomy 升级必须生成 mapping ledger
...
公共资源不是一个可以直接 merge 的大表。先分清 archive、standardized analysis、MAG catalogue 与 taxonomy backbone，再决定下载什么、哪一行才是独立单位。
