...
::: {.callout-tip title="相似度接近百分之百，仍可能需要拒绝判断"}

相似度要与可比较的基因组范围一起解释；低覆盖或小重叠不能支持强同株结论。

:::

## 两个样本的同一物种，能否称为同株？ {#sec-target}
...
[数据与代码清单](https://github.com/petemeng/metagenomics-best-practices/blob/e219a2cd01609cc208a89e291cbd363ed7f2fbd8/examples/50/files.tsv)列出了本例的输入表、分析脚本和环境文件（约 0.2 MiB）。本清单不含原始 FASTQ 或大型数据库；是否需要它们取决于下文的分析起点。清单中的已计算结果仅供核对；读取结果表不等于重新运行统计模型或上游工具。

在新建文件夹中运行下面的 R 代码，下载完成后即可按正文中的相对路径读取文件。需要核验文件版本时，可使用[带完整性检查的下载脚本](https://raw.githubusercontent.com/petemeng/metagenomics-best-practices/62d27927776d6fd61479767fc219d590fb8259d2/examples/50/download.R)。

```{r}
#| label: reader-input-download-50
#| eval: false
options(timeout = 600)
...
### Nucleotide diversity（核苷酸多样性）描述样本内变异 {#sec-theory}
...
### 从“物种检出”升级为 5× genome breadth {#sec-audit}
