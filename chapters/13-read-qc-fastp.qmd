---
title: "读长质控与修剪：FastQC、MultiQC 与 fastp"
author: "Peter"
date: last-modified
format:
  html:
...
::: {.callout-tip title="读段保留率高，为什么仍要检查丢失发生在哪里"}

先看原始质量再选择修剪规则；成对 reads 的损失需要逐项解释，WARN 不能直接当成失败。

:::

## 质控要保留可信读段，而不是把报告变成全绿 {#sec-target}
...
原论文的 Illumina/MGI short-read QC 使用 fastp 0.20.0；Illumina 提供 TruSeq adapters，过滤后最短长度为 45 nt，含一个 `N` 的 reads 和失去配对的 reads 被移除。下面使用同一公开 Illumina run，分别检查过滤前后的质量、读段去向与配对完整性：
...
### 读段保留率高，为什么仍要检查丢失发生在哪里？

本例仅损失 9 对过短 reads，说明当前输入在这组规则下变化很小，并不证明所有样本都可使用同样阈值。总体保留率会掩盖逐样本和 R1/R2 的不平衡；若某组系统性损失更多，其下游检出差异可能部分来自处理。先比较逐样本保留、长度和质量分布，再决定是否需要额外敏感性分析。

修剪还会改变可比对长度和序列证据，不能只以最高保留率或全部 FastQC 变绿为目标。真实群落的组成可产生非均匀 GC 或重复信号，通用 WARN 需要按样本类型解释。数据前缀适合验证流程是否正确计数，不适合估计整个 run 的随机抽样性能。

## 下载本例数据与分析代码 {#sec-reader-inputs}

[数据与代码清单](https://github.com/petemeng/metagenomics-best-practices/blob/e219a2cd01609cc208a89e291cbd363ed7f2fbd8/examples/13/files.tsv)列出了本例的输入表、分析脚本和环境文件（约 4.1 MiB）。本清单不含原始 FASTQ 或大型数据库；是否需要它们取决于下文的分析起点。清单中的已计算结果仅供核对；读取结果表不等于重新运行统计模型或上游工具。

在新建文件夹中运行下面的 R 代码，下载完成后即可按正文中的相对路径读取文件。需要核验文件版本时，可使用[带完整性检查的下载脚本](https://raw.githubusercontent.com/petemeng/metagenomics-best-practices/62d27927776d6fd61479767fc219d590fb8259d2/examples/13/download.R)。

```{r}
#| label: reader-input-download-13
#| eval: false
options(timeout = 600)
...
## 一次性建立同步 FASTQ 前缀 {#sec-code}

### 三个工具回答三个不同问题 {#sec-theory}
