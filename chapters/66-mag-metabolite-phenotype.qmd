---
title: "MAG × 代谢物 / 表型：从基因组代谢潜力到可验证的功能读出"
...
::: {.callout-tip title="多次采样能否弥补只有两位供体"}

预测代谢物与 LC–MS 观测要分开；少数供体的关联不能替代独立功能验证。

:::

## 怎样把 MAG 的代谢潜力与实测表型接起来？ {#sec-target}
...
![Majzoub et al. 2024 Figure 2。Donor 1/2 的预测交集分别为 1,475/1,530，确认交集为 180/176。论文采用 CC BY 4.0。](../figures/66-majzoub-fig2-original.jpg){#fig-66-anchor}
...
11 个纵向代谢组样本能够描述两位供体各自随时间的检出变化，却没有把供体层面的独立样本数从 2 变成 11。供体身份同时关联许多未控制特征，因此不能用供体间模型差异估计可推广的疗效机制，也不能用样本层重采样制造供体层的精度。

模型预测与 LC–MS 检出重叠，说明某些化合物在两种证据层都可见，但测得的代谢物还可能来自饮食、宿主或未建模成员。等量模型输入有助于排除模型数量造成的集合膨胀，却不能消除这些来源歧义。propanoate metabolism 在三种构建下保留，只支持该结果对所比较输入较稳定。

## 下载本例数据与分析代码 {#sec-reader-inputs}

[数据与代码清单](https://github.com/petemeng/metagenomics-best-practices/blob/e219a2cd01609cc208a89e291cbd363ed7f2fbd8/examples/66/files.tsv)列出了本例的输入表、分析脚本和环境文件（约 0.3 MiB）。本清单不含原始 FASTQ 或大型数据库；是否需要它们取决于下文的分析起点。清单中的已计算结果仅供核对；读取结果表不等于重新运行统计模型或上游工具。

在新建文件夹中运行下面的 R 代码，下载完成后即可按正文中的相对路径读取文件。需要核验文件版本时，可使用[带完整性检查的下载脚本](https://raw.githubusercontent.com/petemeng/metagenomics-best-practices/62d27927776d6fd61479767fc219d590fb8259d2/examples/66/download.R)。

```{r}
#| label: reader-input-download-66
#| eval: false
options(timeout = 600)
