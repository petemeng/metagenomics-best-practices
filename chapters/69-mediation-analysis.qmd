...
::: {.callout-tip title="为什么二分类结局不宜直接相乘两个回归系数"}

先确认暴露、中介和结局的时间顺序；再报告重叠、混杂假设与未测混杂敏感性。

:::

## 相关链条什么时候才支持中介解释？ {#sec-target}
...
![Spencer et al. 2021 Figure 3A/B。原图建立 exposure–outcome 锚点](../figures/69-spencer-fig3ab-original.png){#fig-69-anchor}
...
### 为什么二分类结局不宜直接相乘两个回归系数？

非线性模型中，logistic 系数的乘积通常不等于人群平均风险尺度上的间接效应。这里用 g-computation 先计算模型下的结局概率，再在同一人群上标准化，因而能用 percentage points 解释总、直接和间接分量。即便数值可分解，因果解释仍要求正确模型、充分重叠及相应的无未测混杂假设。

间接分量为 +1.0 percentage point，区间跨 0；它与较大的总关联可以同时存在。proportion mediated 是两个不确定量的比值，总效应接近 0 或间接效应方向变化时尤其不稳定。先报告风险尺度的分量及区间，再讨论剩余偏差，比把 4.6% 当成确定机制贡献更合适。

## 下载本例数据与分析代码 {#sec-reader-inputs}

[数据与代码清单](https://github.com/petemeng/metagenomics-best-practices/blob/e219a2cd01609cc208a89e291cbd363ed7f2fbd8/examples/69/files.tsv)列出了本例的输入表、分析脚本和环境文件（约 1.6 MiB）。本清单不含原始 FASTQ 或大型数据库；是否需要它们取决于下文的分析起点。清单中的已计算结果仅供核对；读取结果表不等于重新运行统计模型或上游工具。

在新建文件夹中运行下面的 R 代码，下载完成后即可按正文中的相对路径读取文件。需要核验文件版本时，可使用[带完整性检查的下载脚本](https://raw.githubusercontent.com/petemeng/metagenomics-best-practices/62d27927776d6fd61479767fc219d590fb8259d2/examples/69/download.R)。

```{r}
#| label: reader-input-download-69
#| eval: false
options(timeout = 600)
...
### 先结果核对 checksum、真实交集与 feature rows
...
## 从 225-row LKT table 构造 mediator {#sec-code}

### 先画 DAG，再写回归式 {#sec-theory}
...
### exposure–mediator interaction 不能默认为零
...
## 固定 factor reference 与 adjustment set
...
## 用 Gaussian quadrature 标准化 mediator distribution
...
## positivity 与 measured-confounder overlap
...
A 与 M 相关、M 与 Y 相关，仍不等于中介。用真实纤维—shotgun—免疫治疗结局，拆开 estimand、时间顺序、顺序可交换性、positivity 与未测混杂敏感性。
