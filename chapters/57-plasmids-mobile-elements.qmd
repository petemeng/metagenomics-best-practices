---
author: "Peter"
title: "质粒与可移动元件：从 geNomad 命中到 ARG 可移动性证据"
...
::: {.callout-tip title="候选移动元件与实际转移事件之间还有哪些缺口"}

物理共定位、可移动性推断和已观察到的转移是不同结论；没有直接证据时不宣称转移发生。

:::

## 同一段序列上的基因，能证明已经发生转移吗？ {#sec-target}
...
### 候选移动元件与实际转移事件之间还有哪些缺口？

在 218 个候选条目中观察到 42 个带有转移相关标记，只能说明相应序列具有某类注释证据。候选分类、标记注释、基因共定位和实际转移事件是不同层次的判断；前一层成立，不会自动补齐后一层。

本例主要结果没有支持耐药基因与这些标记的共定位，因此不能把总体计数改写成“发现了耐药传播载体”。组装不完整、来源不明及数据库覆盖不足也应保留为不确定性。结果表应区分已观察到、未观察到和当前数据无法判断，而不是把它们压成一个风险分数。

## 下载参考并验证上游身份 {#sec-code}

### “MGE”不是“质粒”的同义词 {#sec-theory}
...
### 从“跑一个分类器”升级为三层 benchmark {#sec-audit}
...
### transfer-related marker 不等于完整 conjugation system
