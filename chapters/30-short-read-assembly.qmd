---
title: "短读长组装：MEGAHIT vs metaSPAdes、单样本 vs 混合组装"
author: "Peter"
date: last-modified
draft: false
format:
...
::: {.callout-tip title="混合组装的收益，要与输入预算一起解释"}

把连续性、读段回贴与资源代价分开比较；输入量不同的单样本和混合组装不是纯算法对照。

:::

## 组装更连续，是否也更可信？ {#sec-target}
...
[数据与代码清单](https://github.com/petemeng/metagenomics-best-practices/blob/e219a2cd01609cc208a89e291cbd363ed7f2fbd8/examples/30/files.tsv)列出了本例的输入表、分析脚本和环境文件（约 115.1 MiB）。本清单不含原始 FASTQ 或大型数据库；是否需要它们取决于下文的分析起点。清单中的已计算结果仅供核对；读取结果表不等于重新运行统计模型或上游工具。

在新建文件夹中运行下面的 R 代码，下载完成后即可按正文中的相对路径读取文件。需要核验文件版本时，可使用[带完整性检查的下载脚本](https://raw.githubusercontent.com/petemeng/metagenomics-best-practices/62d27927776d6fd61479767fc219d590fb8259d2/examples/30/download.R)。

```{r}
#| label: reader-input-download-30
#| eval: false
options(timeout = 600)
...
## paired fastp，保持两个 mock 的过滤约定一致

### de Bruijn graph 把 reads 改写成 k-mer 邻接问题 {#sec-theory}
