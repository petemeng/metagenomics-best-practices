---
title: "认识数据与平台：FASTQ、短读与长读对全流程的影响"
author: "Peter"
date: last-modified
format:
  html:
...
::: {.callout-tip title="历史平台比较能支持选择原则，不能代替当前验收"}

把读长、准确度与可跨越的序列范围分开看；历史平台的比较结果不能直接代表所有新型号。

:::

## 同一份样本，短读和长读分别保留了什么？ {#sec-target}
...
### 历史平台比较能支持选择原则，不能代替当前验收

同一份 DNA 的平台比较减少了材料差异，却仍包含当时的建库、碱基识别和分析版本。这里三个 run 的总 bases 与误差结构不同，不能仅按 reads 数给平台排序。前 5,000 条记录是确定性文件前缀，适合说明 FASTQ 结构，但不能被当成随机抽样的总体性能估计。

选择数据类型时，应把目标需要的序列跨度与准确度分开考虑：长读能连接更远的上下文，短读可提供另一种准确度与深度配置，但两者都需要与最终终点相匹配的验证。若平台代际或模型版本已变，应重新测量代表性输入，不能把历史 benchmark 的数值直接写进新项目的性能承诺。

## 下载本例数据与分析代码 {#sec-reader-inputs}

[数据与代码清单](https://github.com/petemeng/metagenomics-best-practices/blob/e219a2cd01609cc208a89e291cbd363ed7f2fbd8/examples/08/files.tsv)列出了本例的输入表、分析脚本和环境文件（约 4.5 MiB）。本清单不含原始 FASTQ 或大型数据库；是否需要它们取决于下文的分析起点。清单中的已计算结果仅供核对；读取结果表不等于重新运行统计模型或上游工具。

在新建文件夹中运行下面的 R 代码，下载完成后即可按正文中的相对路径读取文件。需要核验文件版本时，可使用[带完整性检查的下载脚本](https://raw.githubusercontent.com/petemeng/metagenomics-best-practices/62d27927776d6fd61479767fc219d590fb8259d2/examples/08/download.R)。

```{r}
#| label: reader-input-download-08
#| eval: false
options(timeout = 600)
...
### 先修正 accession lineage，不篡改论文 {#sec-audit}
...
### 一个 FASTQ record 必须满足四行语法 {#sec-theory}
