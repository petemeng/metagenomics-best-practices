---
title: "长读长组装：Nanopore / PacBio HiFi 与长读特有问题"
author: "Peter"
date: last-modified
draft: false
format:
...
::: {.callout-tip title="一条很长的 contig，仍可能不是完整正确的基因组"}

连续 contig 和环状信号都需要独立支持；比较平台时同时保留输入预算、错误率与回贴证据。

:::

## 长读组装改善了连续性，还需要检查什么？ {#sec-target}
...
### 一条很长的 contig，仍可能不是完整正确的基因组

长读能跨越短读难以连接的区域，但其误差可能影响基因完整性和后续注释。环状标记也需要跨接头证据与整体一致性支持，不能单凭 assembler 的标签宣布闭环。比较连续性时，应同时保留参考感知错误与回比异常。

本例 ONT 与 HiFi 输入的 bases、读长和历史处理版本不同，因此不能把两列结果解释为只改变平台的因果实验。若新研究的 chemistry 或碱基识别模型改变，旧 benchmark 仍可说明评价维度，却不能直接提供新的性能保证。

## 下载本例数据与分析代码 {#sec-reader-inputs}

[数据与代码清单](https://github.com/petemeng/metagenomics-best-practices/blob/e219a2cd01609cc208a89e291cbd363ed7f2fbd8/examples/31/files.tsv)列出了本例的输入表、分析脚本和环境文件（约 186.6 MiB）。本清单不含原始 FASTQ 或大型数据库；是否需要它们取决于下文的分析起点。清单中的已计算结果仅供核对；读取结果表不等于重新运行统计模型或上游工具。

在新建文件夹中运行下面的 R 代码，下载完成后即可按正文中的相对路径读取文件。需要核验文件版本时，可使用[带完整性检查的下载脚本](https://raw.githubusercontent.com/petemeng/metagenomics-best-practices/62d27927776d6fd61479767fc219d590fb8259d2/examples/31/download.R)。

```{r}
#| label: reader-input-download-31
#| eval: false
options(timeout = 600)
...
### HMW DNA 决定可被跨越的结构上限 {#sec-theory}
...
## Flye 2.9.6 的历史 ONT R9 与 HiFi 分支
