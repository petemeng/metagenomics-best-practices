---
title: "组装质控：QUAST、N50、contig 统计与可用性"
author: "Peter"
date: last-modified
draft: false
format:
...
::: {.callout-tip title="没有精确参考时，哪些质控结论仍然成立"}

连续性不是正确性；把 reference-free 指标与有参考的结构、回收和碱基错误一起判断。

:::

## N50 很高的组装，为什么仍可能不能用？ {#sec-target}
...
QUAST 同时支持无 reference 与有 reference 两种模式；MetaQUAST 再为多基因组混合物提供 combined-reference 和 per-reference 报告 [@gurevich2013quast; @mikheenko2016metaquast]。以下结果来自同一版本的 QUAST 5.3.0 输出。
...
contig 长度、总 bases 和 N50 无需精确参考，但无法单独判断错误连接。若拿近缘参考做 MetaQUAST，真实结构差异也可能被标为异常；因此 reference-aware 结果需要说明参考与样本的关系。没有真值时应结合 reads、覆盖和组装图证据，不能把“无参考可检验”写成“没有错误”。

同样的 bases 和 N50 可以对应不同结构，说明长度摘要不是充分质量指标。gene-centric 分析可能容忍部分碎片化，完整基因组或上下文归属却对结构要求更高。应先定义任务，再设定接受标准；不能给所有用途套一个 N50 门槛。

## 下载本例数据与分析代码 {#sec-reader-inputs}

[数据与代码清单](https://github.com/petemeng/metagenomics-best-practices/blob/e219a2cd01609cc208a89e291cbd363ed7f2fbd8/examples/33/files.tsv)列出了本例的输入表、分析脚本和环境文件（约 742.3 MiB）。本清单不含原始 FASTQ 或大型数据库；是否需要它们取决于下文的分析起点。清单中的已计算结果仅供核对；读取结果表不等于重新运行统计模型或上游工具。

在新建文件夹中运行下面的 R 代码，下载完成后即可按正文中的相对路径读取文件。需要核验文件版本时，可使用[带完整性检查的下载脚本](https://raw.githubusercontent.com/petemeng/metagenomics-best-practices/62d27927776d6fd61479767fc219d590fb8259d2/examples/33/download.R)。

```{r}
#| label: reader-input-download-33
#| eval: false
options(timeout = 600)
...
## 核对上游 bundle，不读取 FASTQ {#sec-code}
...
### N50/L50 是 assembly 自身的长度分布 {#sec-theory}
