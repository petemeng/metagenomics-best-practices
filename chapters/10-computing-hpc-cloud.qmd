---
title: "算力现实：内存、磁盘、HPC、云、Apptainer 与 SLURM"
author: "Peter"
date: last-modified
format:
  html:
...
::: {.callout-tip title="并发上限应由最紧缺的资源决定"}

用真实输入测峰值内存、磁盘和耗时；并发数应服从资源上限，失败续跑要检查输出完整性。

:::

## 先测小规模任务，再申请计算资源 {#sec-target}
...
[数据与代码清单](https://github.com/petemeng/metagenomics-best-practices/blob/e219a2cd01609cc208a89e291cbd363ed7f2fbd8/examples/10/files.tsv)列出了本例的输入表、分析脚本和环境文件（约 4.4 MiB）。本清单不含原始 FASTQ 或大型数据库；是否需要它们取决于下文的分析起点。清单中的已计算结果仅供核对；读取结果表不等于重新运行统计模型或上游工具。

在新建文件夹中运行下面的 R 代码，下载完成后即可按正文中的相对路径读取文件。需要核验文件版本时，可使用[带完整性检查的下载脚本](https://raw.githubusercontent.com/petemeng/metagenomics-best-practices/62d27927776d6fd61479767fc219d590fb8259d2/examples/10/download.R)。

```{r}
#| label: reader-input-download-10
#| eval: false
options(timeout = 600)
...
## 先核对真实输入 footprint {#sec-code}

### 一次作业至少有八个资源维度 {#sec-theory}
...
### 忠实复现资源 benchmark，但不搬运数字 {#sec-audit}
