---
author: "Peter"
title: "病毒发现与质量标准：geNomad / VirSorter2、CheckV、vOTU 与 MIUViG"
...
::: {.callout-tip title="基准集上的检出率能外推到环境样本吗"}

识别结果不等于完整基因组；不同实验类型、质量证据和分类不确定性应分别报告。

:::

## 病毒候选、质量与分类，为什么要分别判断？ {#sec-target}
...
46 条基准序列中，不同工具的并集包含 44 条，高置信结果包含 38 条。这两个数回答的是给定测试集和判定规则下保留了多少候选，而不是实际环境中有多少病毒。若测试集的长度、完整度或来源与目标样本不同，同一个阈值的表现也可能改变。

检出、完整度评估与分类是不同证据层。CheckV 的质量分层不能替代候选发现的准确性评价，无法确定完整度也不等于没有生物学价值。报告时应保留每层的分母和未判定条目，不把筛选后的高置信集合当作环境中全部成员的无偏样本。

## 下载并校验三份小型公开输入 {#sec-code}
...
### 这不是 detector benchmark 的 sensitivity/specificity {#sec-audit}
...
### 发现工具回答“像不像病毒”，不是“有多完整” {#sec-theory}
...
### Library protocol 不是可以事后完全校正的 batch
