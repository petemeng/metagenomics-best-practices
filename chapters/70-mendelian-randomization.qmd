...
::: {.callout-tip title="一致方向与很强的工具变量仍不能保证有效性"}

强度、等位基因协调、多效性和方向性都要检查；BMI 示例的方法结果不能直接外推为微生物因果效应。

:::

## 工具变量能否支持所声称的因果方向？ {#sec-target}
...
### 一致方向与很强的工具变量仍不能保证有效性

minimum F 为 29.9 支持这些 BMI 工具具有相关性，却不检验排除限制。明显的异质性提示工具给出的估计并不一致；剔除离群点可以改变估计，但不会证明保留工具全部有效。leave-one-out 稳定只排除了单个工具完全主导结果的某些情形，也不能排除共同偏差。

把这一流程迁移到微生物组时，暴露定义尤其重要：检出状态、某个比例的变化与绝对丰度不是同一个性状，遗传倾向也不是短期微生物干预。祖源、连锁结构、样本重叠及测量误差都应与效应一起报告。当前 BMI 示例不能提供任何特定微生物的因果效应证据。

## 下载本例数据与分析代码 {#sec-reader-inputs}

[数据与代码清单](https://github.com/petemeng/metagenomics-best-practices/blob/e219a2cd01609cc208a89e291cbd363ed7f2fbd8/examples/70/files.tsv)列出了本例的输入表、分析脚本和环境文件（约 1.1 MiB）。本清单不含原始 FASTQ 或大型数据库；是否需要它们取决于下文的分析起点。清单中的已计算结果仅供核对；读取结果表不等于重新运行统计模型或上游工具。

在新建文件夹中运行下面的 R 代码，下载完成后即可按正文中的相对路径读取文件。需要核验文件版本时，可使用[带完整性检查的下载脚本](https://raw.githubusercontent.com/petemeng/metagenomics-best-practices/62d27927776d6fd61479767fc219d590fb8259d2/examples/70/download.R)。

```{r}
#| label: reader-input-download-70
#| eval: false
options(timeout = 600)
...
## 先按 effect allele 协调，不按 SNP 名直接合并 {#sec-code}

### 三个工具变量假设缺一不可 {#sec-theory}
...
### 微生物 mGWAS 的 instrument validity 尤其脆弱
...
## 异质性、Egger intercept 与 funnel plot 回答不同问题
...
## 二分类结局的 Steiger test 要检查 prevalence
...
MR 不是把微生物丰度换成 SNP 就能谈因果。用真实 BMI→冠心病汇总统计，把工具强度、等位基因协调、多效性、离群位点、方向性与报告边界一次审清。
