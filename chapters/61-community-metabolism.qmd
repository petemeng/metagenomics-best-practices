...
::: {.callout-tip title="丰度加权模型会把测量误差带到哪里"}

预测交换只在给定培养基、丰度权重和优化目标下成立；与实测代谢物或通量分开表述。

:::

## 群落代谢模型中的交换，能怎样解释？ {#sec-target}
...
[数据与代码清单](https://github.com/petemeng/metagenomics-best-practices/blob/e219a2cd01609cc208a89e291cbd363ed7f2fbd8/examples/61/files.tsv)列出了本例的输入表、分析脚本和环境文件（约 7.3 MiB）。本清单不含原始 FASTQ 或大型数据库；是否需要它们取决于下文的分析起点。清单中的已计算结果仅供核对；读取结果表不等于重新运行统计模型或上游工具。

在新建文件夹中运行下面的 R 代码，下载完成后即可按正文中的相对路径读取文件。需要核验文件版本时，可使用[带完整性检查的下载脚本](https://raw.githubusercontent.com/petemeng/metagenomics-best-practices/62d27927776d6fd61479767fc219d590fb8259d2/examples/61/download.R)。

```{r}
#| label: reader-input-download-61
#| eval: false
options(timeout = 600)
...
## 按受试者选择样本并保留原始分母 {#sec-code}

### 从相对丰度到群落模型有五道证据边界 {#sec-theory}
...
### 先检查 sample universe，再看通量 {#sec-audit}
