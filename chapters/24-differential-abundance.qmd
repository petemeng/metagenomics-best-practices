...
先固定协变量和多重检验范围，再区分丰度与检出关联；由相对丰度重建的计数只能用于明确标注的敏感性分析。
...
[数据与代码清单](https://github.com/petemeng/metagenomics-best-practices/blob/e219a2cd01609cc208a89e291cbd363ed7f2fbd8/examples/24/files.tsv)列出了本例的输入表、分析脚本和环境文件（约 1.3 MiB）。本清单不含原始 FASTQ 或大型数据库；是否需要它们取决于下文的分析起点。清单中的已计算结果仅供核对；读取结果表不等于重新运行统计模型或上游工具。

在新建文件夹中运行下面的 R 代码，下载完成后即可按正文中的相对路径读取文件。需要核验文件版本时，可使用[带完整性检查的下载脚本](https://raw.githubusercontent.com/petemeng/metagenomics-best-practices/54d211859b240c610f61d1a2ae72e605ea96f7f3/examples/24/download.R)。
...
## 读表并锁定 110 名独立受试者 {#sec-code}

### “差异”至少有两个生物学问题 {#sec-theory}
...
## ANCOM-BC2 与 pseudo-count sensitivity
...
## ALDEx2 与 denominator sensitivity

### FDR family 必须在看结果前写死
