...
::: {.callout-tip title="曲线趋于平稳，是否就可以停止加深测序"}

用代表性样本检查目标结果是否趋于稳定；物种谱、低丰度基因和 MAG 回收不能共用一个深度标准。

:::

## 测序深度应该由哪个结果决定？ {#sec-target}
...
先写清稳定的是常见物种比例、可检出基因数还是可回收基因组。一个丰度加权覆盖率更重视常见成员，不能保证所有低丰度对象已被充分观察。Lake Lanier 的 68% 是在该数据和模型下的平均覆盖估计，不是“已经发现 68% 的物种”，也不能直接当作人群样本的深度标准。

资源分配还要比较增加独立样本与增加单样本深度的收益。若主要物种谱已稳定而独立重复很少，增加深度不能补足人群异质性；若大多数 reads 被宿主占用，名义总 reads 又会高估可用信息。选择停止规则时应同时报告样本保留、有效微生物序列与目标结果的变化，而不是只引用一个总测序量。

## 下载本例数据与分析代码 {#sec-reader-inputs}

[数据与代码清单](https://github.com/petemeng/metagenomics-best-practices/blob/e219a2cd01609cc208a89e291cbd363ed7f2fbd8/examples/04/files.tsv)列出了本例的输入表、分析脚本和环境文件（约 0.1 MiB）。本清单不含原始 FASTQ 或大型数据库；是否需要它们取决于下文的分析起点。清单中的已计算结果仅供核对；读取结果表不等于重新运行统计模型或上游工具。

在新建文件夹中运行下面的 R 代码，下载完成后即可按正文中的相对路径读取文件。需要核验文件版本时，可使用[带完整性检查的下载脚本](https://raw.githubusercontent.com/petemeng/metagenomics-best-practices/62d27927776d6fd61479767fc219d590fb8259d2/examples/04/download.R)。

```{r}
#| label: reader-input-download-04
#| eval: false
options(timeout = 600)
...
## 读取 768 个 CRC 样本的 reported library size {#sec-code}
...
### 同一个 FASTQ 至少有四种深度分母 {#sec-theory}
...
### 从固定 reads 配额升级为“每个终点的有效分母” {#sec-audit}
