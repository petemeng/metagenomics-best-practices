...
::: {.callout-tip title="蛋白推断的证据门槛为何会改变下游结论"}

PSM、蛋白推断与物种归属不能跳步；共享肽段和搜索库决定结论可以细化到哪一层。

:::

## 从肽段到物种，哪些归属仍有歧义？ {#sec-target}
...
![Lloyd-Price et al. 2019 Supplementary Figure 1，第 1 页。论文与补充材料采用 CC BY 4.0。](../figures/65-lloyd-price-supp-fig1-page1-original.png){#fig-65-anchor}
...
从 66,606 个宽松条件下检出的 protein IDs，到严格条件下的 10,469 个，变化来自证据定义。PSM、肽段与 protein group 的错误率针对不同层级，不能把其中一个 1% 数字套到所有下游对象。共享肽段带来的归属歧义也不会因重复计算到多个物种而消失。

DNA–RNA 与 RNA–protein 的相关程度不同，可能同时受到生物学时间尺度、检测动态范围和缺失机制影响。只分析三层都检出的特征，还会选择更易检测的部分。跨层不一致值得检查，但不能直接认定为翻译调控；跨层一致也不能直接认定酶活性或通量增加。

## 下载本例数据与分析代码 {#sec-reader-inputs}

[数据与代码清单](https://github.com/petemeng/metagenomics-best-practices/blob/e219a2cd01609cc208a89e291cbd363ed7f2fbd8/examples/65/files.tsv)列出了本例的输入表、分析脚本和环境文件（约 10.7 MiB）。本清单不含原始 FASTQ 或大型数据库；是否需要它们取决于下文的分析起点。清单中的已计算结果仅供核对；读取结果表不等于重新运行统计模型或上游工具。

在新建文件夹中运行下面的 R 代码，下载完成后即可按正文中的相对路径读取文件。需要核验文件版本时，可使用[带完整性检查的下载脚本](https://raw.githubusercontent.com/petemeng/metagenomics-best-practices/62d27927776d6fd61479767fc219d590fb8259d2/examples/65/download.R)。

```{r}
#| label: reader-input-download-65
#| eval: false
options(timeout = 600)
...
## 先建立带来源标签的 metagenome-informed FASTA {#sec-code}
...
### PSM、peptide 与 protein group 是三个不同层级 {#sec-theory}
