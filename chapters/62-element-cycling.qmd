...
::: {.callout-tip title="通路公式的 AND 与 OR 会改变什么结论"}

区分单基因、完整过程和基因组载体；目录回收不全时，不能把局部 MAG 结果当作整个群落。

:::

## 元素循环潜力，能否只看一个标记基因？ {#sec-target}
...
### 通路公式的 AND 与 OR 会改变什么结论？

通路中互相替代的反应与缺一不可的步骤，应采用不同的聚合逻辑。简单相加可能让某个丰度很高的标记掩盖必要步骤缺失；严格门控则可能因测序或注释漏检而把真实但证据不全的过程标为未检出。因此，连续指数与完整标记判定应并排报告，分别表示数量信息与证据完整性。

500 份样本来自 56 个温泉，环境比较的有效独立单位不能按 500 计算。以温泉为单位概括避免伪重复，但同时丢失地点内部的变化；结论应明确针对地点间梯度。MAG 层再受回收范围限制，不能把已恢复基因组中的过程占比解释为全群落比例或实测元素转化速率。

## 下载本例数据与分析代码 {#sec-reader-inputs}

[数据与代码清单](https://github.com/petemeng/metagenomics-best-practices/blob/e219a2cd01609cc208a89e291cbd363ed7f2fbd8/examples/62/files.tsv)列出了本例的输入表、分析脚本和环境文件（约 22.9 MiB）。本清单不含原始 FASTQ 或大型数据库；是否需要它们取决于下文的分析起点。清单中的已计算结果仅供核对；读取结果表不等于重新运行统计模型或上游工具。

在新建文件夹中运行下面的 R 代码，下载完成后即可按正文中的相对路径读取文件。需要核验文件版本时，可使用[带完整性检查的下载脚本](https://raw.githubusercontent.com/petemeng/metagenomics-best-practices/62d27927776d6fd61479767fc219d590fb8259d2/examples/62/download.R)。

```{r}
#| label: reader-input-download-62
#| eval: false
options(timeout = 600)
...
## 读取 68 个预登记 markers，同时检查完整表宽度 {#sec-code}

### 同一过程需要两个不同对象 {#sec-theory}
