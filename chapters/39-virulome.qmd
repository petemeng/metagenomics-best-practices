...
::: {.callout-tip title="同源功能与致病表型之间还有哪些证据缺口"}
...
数据库中的参考条目可能有实验支持，但将其标签转移到一个相似序列，仍是一项预测。相似的黏附、运动或调控相关基因也可能服务于一般生存功能；单个命中不能给整个样本或菌株贴上致病标签。

改变参考范围后候选增加，首先说明可搜索知识集合改变，不是样本变得更危险。报告时应把参考证据、样本序列支持和表型结论分开，并明确无法从当前数据判断的部分。不要将候选数量、相对丰度或数据库类别直接合成为临床危害评分。
