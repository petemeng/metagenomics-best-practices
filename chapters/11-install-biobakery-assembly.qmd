---
title: "安装 bioBakery、组装分箱工具链与数据库"
author: "Peter"
date: last-modified
format:
  html:
...
::: {.callout-tip title="软件锁与数据库锁必须一起定义结果版本"}

工具能启动不等于数据库已就绪；把软件版本、数据库 release 和入口测试分别记录。

:::

## 软件与数据库必须分别确认版本 {#sec-target}
...
同一个工具版本读取不同 marker 或参考基因集，可能输出不同的可检出对象。因此“环境能安装”与“分析可以重建”不是同一个结论；后者还需要数据库 release、实际文件身份与关键参数。本例工具入口曾与包管理器记录不一致，也说明应检查运行时，而不能只保存安装清单。

升级应作为新的分析版本进行比较：先在固定输入上检查对象数量、未分类比例与关键结果，再决定是否迁移全队列。不要让部分样本读取旧数据库、部分读取新数据库后直接合并。下载失败或缺少必要文件时应明确停止，不能静默切换到一个不同的参考版本。

## 下载本例数据与分析代码 {#sec-reader-inputs}

[数据与代码清单](https://github.com/petemeng/metagenomics-best-practices/blob/e219a2cd01609cc208a89e291cbd363ed7f2fbd8/examples/11/files.tsv)列出了本例的输入表、分析脚本和环境文件（约 0.2 MiB）。本清单不含原始 FASTQ 或大型数据库；是否需要它们取决于下文的分析起点。清单中的已计算结果仅供核对；读取结果表不等于重新运行统计模型或上游工具。

在新建文件夹中运行下面的 R 代码，下载完成后即可按正文中的相对路径读取文件。需要核验文件版本时，可使用[带完整性检查的下载脚本](https://raw.githubusercontent.com/petemeng/metagenomics-best-practices/62d27927776d6fd61479767fc219d590fb8259d2/examples/11/download.R)。

```{r}
#| label: reader-input-download-11
#| eval: false
options(timeout = 600)
...
## 先拒绝隐形 channel 与 user site {#sec-code}
...
### 检查 1：以当日官方指针为准，但运行时不用指针 {#sec-audit}
