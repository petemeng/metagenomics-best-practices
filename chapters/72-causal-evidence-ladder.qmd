...
::: {.callout-tip title="一项干预有效，并不意味着其中每个机制都已证实"}

不同设计的证据不能简单相加；每项结果都要对应同一个具体主张，并保留尚未测试的环节。

:::

## 哪些证据能把关联推进到因果判断？ {#sec-target}
...
### 一项干预有效，并不意味着其中每个机制都已证实

随机研究直接识别的是被分配的干预整体及其对照条件下的效应。若干预同时改变多个成分，就不能从整体效应反推出其中某个成员单独必要或充分。来自另一研究系统的机制线索可以帮助解释，但研究对象、干预和结局不同，不能将它们拼成一个从未直接估计的效应。

三角验证的价值在于不同设计具有不同的主要偏差，而不是支持性文章数量多。若若干结果共享选择偏差、测量误差或同一个未控制因素，方向一致仍可能来自共同原因。阅读证据时，应分别写清已支持的窄结论、尚未检验的环节，以及改变结论所需的证据类型。

## 把 publication metadata 与人工证据提取分开 {#sec-code}
...
### 先写 claim contract，再谈证据强弱 {#sec-theory}
