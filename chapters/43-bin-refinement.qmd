---
author: "Peter"
title: "Bin 精炼与整合：DAS Tool 与 Binette"
...
::: {.callout-tip title="精炼后的质量必须针对最终序列重新计算"}

记录每个最终 bin 的来源和 contig 成员；选择不能借用已知真值，质量必须对最终序列重新计算。

:::

## 合并分箱结果，为什么不能把好指标拼在一起？ {#sec-target}
...
split 或 merge 改变了 contig 集合，旧 bin 的最佳 completeness 与最低 contamination 不能拼成新 bin 的质量报告。最终序列需要重新评价，并保留每条 contig 来自哪个候选、是否重复进入多个结果。这样才能区分真正改善与同一序列被重复计算。

Binette 用 CheckM2 参与选择，随后再次用 CheckM2 评分主要验证最终对象的一致性，不是完全独立的方法验证。GUNC、其他质量维度及留到最后的 mock 真值提供不同信息。应把选择用的标准与最终评价用的证据分开说明，不以一个总分替代全部质量检查。

## 下载本例数据与分析代码 {#sec-reader-inputs}

[数据与代码清单](https://github.com/petemeng/metagenomics-best-practices/blob/e219a2cd01609cc208a89e291cbd363ed7f2fbd8/examples/43/files.tsv)列出了本例的输入表、分析脚本和环境文件（约 2.7 MiB）。本清单不含原始 FASTQ 或大型数据库；是否需要它们取决于下文的分析起点。清单中的已计算结果仅供核对；读取结果表不等于重新运行统计模型或上游工具。

在新建文件夹中运行下面的 R 代码，下载完成后即可按正文中的相对路径读取文件。需要核验文件版本时，可使用[带完整性检查的下载脚本](https://raw.githubusercontent.com/petemeng/metagenomics-best-practices/62d27927776d6fd61479767fc219d590fb8259d2/examples/43/download.R)。

```{r}
#| label: reader-input-download-43
#| eval: false
options(timeout = 600)
...
## 从 frozen membership 精确重建输入 bins {#sec-code}

### 输入是 partitions，不是互不相关的 FASTA 文件 {#sec-theory}
...
### 忠实复现 DAS Tool，再用现代 QC 复核 {#sec-audit}
