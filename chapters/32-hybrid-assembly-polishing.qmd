---
title: "Hybrid assembly 与 polishing：何时混合，何时直接 HiFi"
author: "Peter"
date: last-modified
draft: false
format:
...
::: {.callout-tip title="纠错改善局部碱基，不一定修复结构错误"}

在同一输入范围内比较纠错前后的一致性错误和结构错误；不要只以 N50 决定是否接受结果。

:::

## 短读纠错能否改善长读组装？ {#sec-target}
...
[数据与代码清单](https://github.com/petemeng/metagenomics-best-practices/blob/e219a2cd01609cc208a89e291cbd363ed7f2fbd8/examples/32/files.tsv)列出了本例的输入表、分析脚本和环境文件（约 528.1 MiB）。本清单不含原始 FASTQ 或大型数据库；是否需要它们取决于下文的分析起点。清单中的已计算结果仅供核对；读取结果表不等于重新运行统计模型或上游工具。

在新建文件夹中运行下面的 R 代码，下载完成后即可按正文中的相对路径读取文件。需要核验文件版本时，可使用[带完整性检查的下载脚本](https://raw.githubusercontent.com/petemeng/metagenomics-best-practices/62d27927776d6fd61479767fc219d590fb8259d2/examples/32/download.R)。

```{r}
#| label: reader-input-download-32
#| eval: false
options(timeout = 600)
...
### short-read-first 与 long-read-first 是两种因果顺序 {#sec-theory}
