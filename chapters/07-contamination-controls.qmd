---
title: "污染与对照：blank、kitome、index hopping、decontam"
author: "Peter"
date: last-modified
format:
  html:
...
::: {.callout-tip title="不同污染判据冲突时，先检查对照覆盖了哪一批"}

过程空白、浓度趋势和逐样本污染负担要一起看；不要仅凭通用菌名黑名单删除特征。

:::

## 低生物量样本里，哪些信号来自污染？ {#sec-target}
...
### 不同污染判据冲突时，先检查对照覆盖了哪一批

frequency 方法依赖 DNA 浓度能反映相关样本成分；宿主 DNA 占比变化很大时，总浓度未必是微生物量的好代理。prevalence 方法则依赖与样本匹配的阴性对照。本例的 30 个对照分布在六块板上，合并后得到的候选清单还应与板内证据对照，避免把批次特有背景稀释掉。

同一个分类单元可以同时来自真实样本与背景污染，所以被标记不等于应在所有样本中无条件归零。应观察逐样本污染负担、主要结果对处理规则的敏感性，并说明无法补做的对照缺口。对于低生物量样本，扩大软件过滤力度不能替代采样与流程对照。

## 下载本例数据与分析代码 {#sec-reader-inputs}

[数据与代码清单](https://github.com/petemeng/metagenomics-best-practices/blob/e219a2cd01609cc208a89e291cbd363ed7f2fbd8/examples/07/files.tsv)列出了本例的输入表、分析脚本和环境文件（约 5.0 MiB）。本清单不含原始 FASTQ 或大型数据库；是否需要它们取决于下文的分析起点。清单中的已计算结果仅供核对；读取结果表不等于重新运行统计模型或上游工具。

在新建文件夹中运行下面的 R 代码，下载完成后即可按正文中的相对路径读取文件。需要核验文件版本时，可使用[带完整性检查的下载脚本](https://raw.githubusercontent.com/petemeng/metagenomics-best-practices/62d27927776d6fd61479767fc219d590fb8259d2/examples/07/download.R)。

```{r}
#| label: reader-input-download-07
#| eval: false
options(timeout = 600)
...
## 读取三件套并锁定 byte-level lineage {#sec-code}

### 观测表是多种来源的叠加 {#sec-theory}
...
## 分开运行 frequency、prevalence 与 combined

### blank 是一条定位链，不是一个名为“blank”的样本
...
### Shotgun feature table 要换分辨率，不换证据逻辑
...
### 把浓度从“总 DNA”升级为 endpoint-relevant load
...
### 用 negative、positive 与 alien controls 组成闭环
...
## 在送测前检查 UDI，在收数后索要 index-pair matrix

### index hopping 的识别对象是“索引组合”，不是 taxonomy
...
### Index hopping 还要与 wet-lab cross-contamination 区分
...
### 把 shotgun 结果整理成 integer count matrix
...
### Feature prevalence 与 combined classification
...
### Sample burden 与 DNA concentration
