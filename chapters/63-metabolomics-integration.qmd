---
title: "与代谢组整合：Procrustes / Mantel / HAllA / sPLS-DIABLO"
...
## 微生物与代谢物相关，是否来自同一个过程？ {#sec-target}
...
[数据与代码清单](https://github.com/petemeng/metagenomics-best-practices/blob/e219a2cd01609cc208a89e291cbd363ed7f2fbd8/examples/63/files.tsv)列出了本例的输入表、分析脚本和环境文件（约 8.5 MiB）。本清单不含原始 FASTQ 或大型数据库；是否需要它们取决于下文的分析起点。清单中的已计算结果仅供核对；读取结果表不等于重新运行统计模型或上游工具。

在新建文件夹中运行下面的 R 代码，下载完成后即可按正文中的相对路径读取文件。需要核验文件版本时，可使用[带完整性检查的下载脚本](https://raw.githubusercontent.com/petemeng/metagenomics-best-practices/54d211859b240c610f61d1a2ae72e605ea96f7f3/examples/63/download.R)。

```{r}
#| label: reader-input-download-63
#| eval: false
options(timeout = 600)
...
## 锁定 discovery 过滤、样本顺序和转换 {#sec-code}

### 第一条门是 sample alignment，不是相关系数 {#sec-theory}
...
### 从并排 ordination 升级到三个明确 estimands {#sec-audit}
