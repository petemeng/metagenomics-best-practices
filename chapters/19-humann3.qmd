---
title: "HUMAnN3：gene family、pathway abundance 与 pathway coverage"
author: "Peter"
date: last-modified
draft: false
format:
...
::: {.callout-tip title="物种贡献之和为什么不总等于群落通路丰度"}

先确认 feature 类型和单位；pathway coverage 不等于 abundance，物种分层行也不能重复计入总量。

:::

## 基因、反应和通路表，为什么不能混着读？ {#sec-target}
...
gene family 的群落行与分层行具有相加关系，但通路由多个反应及重建规则决定；先在整个群落重建与先在各物种中重建再求和，不是同一运算。若把所有 pathway strata 加回 community 行，可能重复计数或得到不成立的恒等式。

本例同时保留基因家族、反应、通路 abundance 与 coverage，正是为了分别回答“检测到哪些序列功能”“如何组成通路”和“证据覆盖到什么程度”。coverage 高不等于通路活跃，abundance 高也不证明每个必要反应都有直接证据。进入差异分析前，应先锁定其中一种测量对象。

## 下载本例数据与分析代码 {#sec-reader-inputs}

[数据与代码清单](https://github.com/petemeng/metagenomics-best-practices/blob/e219a2cd01609cc208a89e291cbd363ed7f2fbd8/examples/19/files.tsv)列出了本例的输入表、分析脚本和环境文件（约 7.4 MiB）。本清单不含原始 FASTQ 或大型数据库；是否需要它们取决于下文的分析起点。清单中的已计算结果仅供核对；读取结果表不等于重新运行统计模型或上游工具。

在新建文件夹中运行下面的 R 代码，下载完成后即可按正文中的相对路径读取文件。需要核验文件版本时，可使用[带完整性检查的下载脚本](https://raw.githubusercontent.com/petemeng/metagenomics-best-practices/62d27927776d6fd61479767fc219d590fb8259d2/examples/19/download.R)。

```{r}
#| label: reader-input-download-19
#| eval: false
options(timeout = 600)
...
## 串接 mates，但先核对 199,982 条唯一 read IDs {#sec-code}
...
### Taxonomic prescreen 决定 sample-specific nucleotide database {#sec-theory}
...
### 兼容性要由 parser 与数据库版本共同证明 {#sec-audit}
