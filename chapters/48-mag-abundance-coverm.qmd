---
author: "Peter"
title: "MAG 丰度定量：CoverM 的 depth、breadth 与 relative abundance"
...
::: {.callout-tip title="目录内百分比与整个群落的份额需要并列"}

先报告目录回收范围，再解释目录内相对丰度；平均深度与检出需要不同的覆盖标准。

:::

## MAG 丰度表漏掉了多少群落信息？ {#sec-target}
...
本例 catalog 捕获约 70% reads，目录中的相对丰度只描述这个可回收范围。剩余部分可能来自未组装、未分箱、参考外成员或比对限制；不能把它们自动归成不存在的物种。跨样本比较时，capture 改变会影响条件分母，需与丰度图同时展示。

mean depth 与 breadth 决定能否可靠检测，竞争性比对规则则影响近缘代表之间如何分配 reads。提高 identity 后总 capture 降低而主要相对丰度变化较小，是本例的敏感性结果，不是所有数据都稳健的保证。这里也没有独立细胞负荷，所以不能把覆盖度百分比解释为每克样本的数量。

## 下载本例数据与分析代码 {#sec-reader-inputs}

[数据与代码清单](https://github.com/petemeng/metagenomics-best-practices/blob/e219a2cd01609cc208a89e291cbd363ed7f2fbd8/examples/48/files.tsv)列出了本例的输入表、分析脚本和环境文件（约 0.2 MiB）。本清单不含原始 FASTQ 或大型数据库；是否需要它们取决于下文的分析起点。清单中的已计算结果仅供核对；读取结果表不等于重新运行统计模型或上游工具。

在新建文件夹中运行下面的 R 代码，下载完成后即可按正文中的相对路径读取文件。需要核验文件版本时，可使用[带完整性检查的下载脚本](https://raw.githubusercontent.com/petemeng/metagenomics-best-practices/62d27927776d6fd61479767fc219d590fb8259d2/examples/48/download.R)。

```{r}
#| label: reader-input-download-48
#| eval: false
options(timeout = 600)
...
## 解压 representatives 并核对 reads {#sec-code}
...
## 计算 detection 与 catalog capture

### Mean depth：平均堆叠高度 {#sec-theory}
...
### Covered fraction：支持是否铺满 genome
...
### 从单个 MAG 分别回贴升级为 catalog-wide competitive mapping {#sec-audit}
...
MAG 定量必须把 identity、breadth、depth、relative abundance 与 catalog capture 一起报告，mapped reads 不能单独代表丰度。
