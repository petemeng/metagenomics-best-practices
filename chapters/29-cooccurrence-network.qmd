---
title: "共现网络 + 网络进阶（鲁棒性 / keystone / Zi-Pi）"
author: "Peter"
date: last-modified
draft: false
format:
...
![Kurtz et al. 2015 Figure 2 原图。右半部分给出 CLR、MB/glasso 与 StARS 的方法骨架。原图由 PLOS 以 CC BY 4.0 发布，未改动；官方 large PNG 为 3600 × 2313 px，本地](../figures/29-kurtz-fig2-original.png){#fig-29-anchor}
...
70 条主网络边中只有 30 条在 bootstrap 中达到 70% 选择率，说明稀疏图本身仍有不确定性。StARS 的平均稳定性目标不等于每条边的错误率控制；换过滤、变换或调整集造成的差异，也不能由同一设置下更多 bootstrap 消除。

这里先将 500 个样本汇总为 56 个等权温泉，并调整 region、温度和 pH，减少了几种明确的结构影响，但未知环境与目录选择仍可能造成关联。删除图节点后的连通性变化描述估计图，不代表真实群落会如何恢复。五个拓扑候选更适合用来提出后续问题，而不是被直接命名为生态 keystone。

## 下载本例数据与分析代码 {#sec-reader-inputs}

[数据与代码清单](https://github.com/petemeng/metagenomics-best-practices/blob/e219a2cd01609cc208a89e291cbd363ed7f2fbd8/examples/29/files.tsv)列出了本例的输入表、分析脚本和环境文件（约 1.1 MiB）。本清单不含原始 FASTQ 或大型数据库；是否需要它们取决于下文的分析起点。清单中的已计算结果仅供核对；读取结果表不等于重新运行统计模型或上游工具。

在新建文件夹中运行下面的 R 代码，下载完成后即可按正文中的相对路径读取文件。需要核验文件版本时，可使用[带完整性检查的下载脚本](https://raw.githubusercontent.com/petemeng/metagenomics-best-practices/62d27927776d6fd61479767fc219d590fb8259d2/examples/29/download.R)。

```{r}
#| label: reader-input-download-29
#| eval: false
options(timeout = 600)
...
::: {.callout-tip title="稳定性筛选不能把网络边变成互作证据"}
...
### 先忠实保留 SPIEC-EASI 的方法问题，再说明实现差异 {#sec-audit}
...
### pairwise correlation 会把间接路径也画成边 {#sec-theory}
