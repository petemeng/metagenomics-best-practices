...
::: {.callout-tip title="ANI 很高，也要看比较了多少序列"}

聚类边界和代表选择规则都要写清楚；代表基因组不能代替簇内菌株差异。

:::

## 去冗余之后，一个代表基因组代表了什么？ {#sec-target}
...
[数据与代码清单](https://github.com/petemeng/metagenomics-best-practices/blob/e219a2cd01609cc208a89e291cbd363ed7f2fbd8/examples/45/files.tsv)列出了本例的输入表、分析脚本和环境文件（约 17.1 MiB）。本清单不含原始 FASTQ 或大型数据库；是否需要它们取决于下文的分析起点。清单中的已计算结果仅供核对；读取结果表不等于重新运行统计模型或上游工具。

在新建文件夹中运行下面的 R 代码，下载完成后即可按正文中的相对路径读取文件。需要核验文件版本时，可使用[带完整性检查的下载脚本](https://raw.githubusercontent.com/petemeng/metagenomics-best-practices/62d27927776d6fd61479767fc219d590fb8259d2/examples/45/download.R)。

```{r}
#| label: reader-input-download-45
#| eval: false
options(timeout = 600)
...
### ANI 与 alignment fraction 是一对门槛 {#sec-theory}
...
### 从“把所有 HQ MAG 放一起”升级为统一 gate 后聚类 {#sec-audit}
