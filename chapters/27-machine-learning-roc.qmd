---
title: "随机森林 / 梯度提升分类 + ROC + 重要性 + 防过拟合规范"
author: "Peter"
date: last-modified
draft: false
format:
...
### 更高的内部 AUC，是否意味着更好的实际使用价值？

两个模型必须在相同外层划分与相同样本上比较，否则性能差可能只是划分难度不同。重复交叉验证的预测并非新增独立病例；应按真实受试者汇总，并保留配对的不确定性。若根据外层测试结果不断改超参数网格，外层测试也会逐渐参与选择。

114 人的病例–对照队列适合内部方法评估，不代表筛查人群的患病率。实际使用还需要外部校准、预定阈值及成本收益判断；AUC 不提供这些信息。高重要性物种可能是相关特征的代理，应连同稳定性、原始分布和合理基线模型解释。

## 下载本例数据与分析代码 {#sec-reader-inputs}

[数据与代码清单](https://github.com/petemeng/metagenomics-best-practices/blob/e219a2cd01609cc208a89e291cbd363ed7f2fbd8/examples/27/files.tsv)列出了本例的输入表、分析脚本和环境文件（约 0.8 MiB）。本清单不含原始 FASTQ 或大型数据库；是否需要它们取决于下文的分析起点。清单中的已计算结果仅供核对；读取结果表不等于重新运行统计模型或上游工具。

在新建文件夹中运行下面的 R 代码，下载完成后即可按正文中的相对路径读取文件。需要核验文件版本时，可使用[带完整性检查的下载脚本](https://raw.githubusercontent.com/petemeng/metagenomics-best-practices/62d27927776d6fd61479767fc219d590fb8259d2/examples/27/download.R)。

```{r}
#| label: reader-input-download-27
#| eval: false
options(timeout = 600)
...
::: {.callout-tip title="更高的内部 AUC，是否意味着更好的实际使用价值"}

筛选、变换和调参都放进训练折；同时报告区分度、校准和测试折重要性，不只展示 ROC。

:::

## 模型的高分，能否经得起真正的留出测试？ {#sec-target}
...
![Zeller et al. 2014 Figure 1。原研究用粪便宏基因组构建 CRC 分类器，并设置独立法国队列验证。原图按 CC BY 4.0 使用，](../figures/27-zeller-fig1-original.png){#fig-27-anchor}
