---
author: "Peter"
title: "StrainPhlAn：marker 基因菌株系统发育"
...
::: {.callout-tip title="marker 树描述的是可见标记区的谱系关系"}

树拓扑依赖 marker 覆盖与缺失模式；地理或分组聚集不自动证明传播方向。

:::

## 菌株树上的聚集，能支持多强的结论？ {#sec-target}
...
### marker 树描述的是可见标记区的谱系关系

25 个样本分属 13 项研究和 9 个国家，国家与研究高度重合。树上同国聚集可以来自真实群体结构，也可能包含研究与流程差异；不能仅靠颜色聚集解释为地理原因。样本对共享参与者和研究背景，距离点也不是全部独立重复。

marker 缺失模式会改变共同可比较范围，物种内混合还可能让共识不代表单一克隆。应查看比对覆盖和分支支持，再解释局部聚类。树上的近距离关系提供共享线索，不提供方向、接触或具体传播事件证据。

## 下载本例数据与分析代码 {#sec-reader-inputs}

[数据与代码清单](https://github.com/petemeng/metagenomics-best-practices/blob/e219a2cd01609cc208a89e291cbd363ed7f2fbd8/examples/51/files.tsv)列出了本例的输入表、分析脚本和环境文件（约 1.4 MiB）。本清单不含原始 FASTQ 或大型数据库；是否需要它们取决于下文的分析起点。清单中的已计算结果仅供核对；读取结果表不等于重新运行统计模型或上游工具。

在新建文件夹中运行下面的 R 代码，下载完成后即可按正文中的相对路径读取文件。需要核验文件版本时，可使用[带完整性检查的下载脚本](https://raw.githubusercontent.com/petemeng/metagenomics-best-practices/62d27927776d6fd61479767fc219d590fb8259d2/examples/51/download.R)。

```{r}
#| label: reader-input-download-51
#| eval: false
options(timeout = 600)
...
### 每个样本贡献一套物种特异 consensus markers {#sec-theory}
...
## 说明 marker 筛选与树的支持度 {#sec-methods}
