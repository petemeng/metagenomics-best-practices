---
title: "从样本到文库：DNA 提取、建库偏差与定量宏基因组"
author: "Peter"
date: last-modified
format:
  html:
...
::: {.callout-tip title="校准总量与校正类群偏差是两个问题"}

相对丰度不能代替细胞数；外部定量参照的加入位置，决定它能够校正哪一段流程。

:::

## 提取偏差与绝对数量，需要分别测量 {#sec-target}
...
一个外部参照可以恢复样本间总量尺度，却不一定修正每个类群不同的回收效率。本例同一协议下的成对偏差已很大，因此即使总体相关很高，也不能据此认为每个菌的定量都准确。应逐类群检查误差方向与范围，并明确校准针对的是 DNA、基因拷贝还是细胞。

将某个 mock 上得到的校准直接用于不同基质或不同流程，隐含了偏差可迁移的假设。若条件改变，应保留独立对照证据；缺少这种证据时，报告相应的校准单位和范围，不把结果无条件命名为活细胞数。

## 下载本例数据与分析代码 {#sec-reader-inputs}

[数据与代码清单](https://github.com/petemeng/metagenomics-best-practices/blob/e219a2cd01609cc208a89e291cbd363ed7f2fbd8/examples/05/files.tsv)列出了本例的输入表、分析脚本和环境文件（约 0.0 MiB）。本清单不含原始 FASTQ 或大型数据库；是否需要它们取决于下文的分析起点。清单中的已计算结果仅供核对；读取结果表不等于重新运行统计模型或上游工具。

在新建文件夹中运行下面的 R 代码，下载完成后即可按正文中的相对路径读取文件。需要核验文件版本时，可使用[带完整性检查的下载脚本](https://raw.githubusercontent.com/petemeng/metagenomics-best-practices/62d27927776d6fd61479767fc219d590fb8259d2/examples/05/download.R)。

```{r}
#| label: reader-input-download-05
#| eval: false
options(timeout = 600)
...
## 读取并验证 27 个真实 mock profiles {#sec-code}
...
### 忠实复算历史 benchmark，但不把旧流程当成当前数据库规范 {#sec-audit}
