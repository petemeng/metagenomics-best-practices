---
author: "Peter"
title: "真核微生物：EukDetect2 与 EukRep"
...
::: {.callout-tip title="真核 reads 检出与 contig 分类如何相互补充"}

两种输入回答不同层级的问题；DNA 检出不等于活性或定植，也不能忽略参考库覆盖。

:::

## 真核信号来自 reads 还是 contigs？ {#sec-target}
...
[数据与代码清单](https://github.com/petemeng/metagenomics-best-practices/blob/e219a2cd01609cc208a89e291cbd363ed7f2fbd8/examples/58/files.tsv)列出了本例的输入表、分析脚本和环境文件（约 4.6 MiB）。本清单不含原始 FASTQ 或大型数据库；是否需要它们取决于下文的分析起点。清单中的已计算结果仅供核对；读取结果表不等于重新运行统计模型或上游工具。

在新建文件夹中运行下面的 R 代码，下载完成后即可按正文中的相对路径读取文件。需要核验文件版本时，可使用[带完整性检查的下载脚本](https://raw.githubusercontent.com/petemeng/metagenomics-best-practices/62d27927776d6fd61479767fc219d590fb8259d2/examples/58/download.R)。

```{r}
#| label: reader-input-download-58
#| eval: false
options(timeout = 600)
...
## 下载并通过 bytes + MD5 gate {#sec-code}

### 为什么真核微生物容易在常规宏基因组流程里消失 {#sec-theory}
...
### 先忠实复现，再区分 EukDetect 与 EukDetect2 {#sec-audit}
