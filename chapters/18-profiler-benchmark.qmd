---
title: "谱工具横评：MetaPhlAn vs Kraken2/Bracken vs mOTUs"
author: "Peter"
date: last-modified
draft: false
format:
...
::: {.callout-tip title="共同物种域让比较公平，也缩小了结论范围"}

先对齐工具能够识别的共同物种集合；相关系数高，仍可能同时存在丰度偏差和漏检。

:::

## 不同工具给出不同物种，先比较哪一部分？ {#sec-target}
...
三工具严格共同的一对一物种域只有 52 个物种，占预期组成质量的 70.738%。在这个范围比较误差可以减少命名与覆盖差异，但也排除了较难对齐的对象。应同时报告各工具完整参考范围的表现，不能把共同域内最好者直接称为所有群落的最佳工具。

组成相关很高仍可能存在系统性比例偏差，丰度加权误差较小也可能漏掉很多稀有成员。因此选择指标应对应主要问题：群落总体组成、可靠检出或某个预设目标并不等价。这个单一真实 mock 支持受控比较，不提供跨生态环境的永久排名。

## 下载本例数据与分析代码 {#sec-reader-inputs}

[数据与代码清单](https://github.com/petemeng/metagenomics-best-practices/blob/e219a2cd01609cc208a89e291cbd363ed7f2fbd8/examples/18/files.tsv)列出了本例的输入表、分析脚本和环境文件（约 0.2 MiB）。本清单不含原始 FASTQ 或大型数据库；是否需要它们取决于下文的分析起点。清单中的已计算结果仅供核对；读取结果表不等于重新运行统计模型或上游工具。

在新建文件夹中运行下面的 R 代码，下载完成后即可按正文中的相对路径读取文件。需要核验文件版本时，可使用[带完整性检查的下载脚本](https://raw.githubusercontent.com/petemeng/metagenomics-best-practices/62d27927776d6fd61479767fc219d590fb8259d2/examples/18/download.R)。

```{r}
#| label: reader-input-download-18
#| eval: false
options(timeout = 600)
...
## 先保存 profiler design，不让默认与升级分支混在一起 {#sec-code}

### 一个“物种名”背后至少有四种边界 {#sec-theory}
...
### 忠实保留默认结果，再增加预先声明的敏感性分支 {#sec-audit}
