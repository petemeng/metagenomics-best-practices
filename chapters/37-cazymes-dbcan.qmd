---
title: "CAZymes 碳水活性酶：dbCAN 共识、底物与 CGC"
author: "Peter"
date: last-modified
draft: false
format:
...
::: {.callout-tip title="多工具共识减少分歧，却不是独立功能验证"}

先区分家族级命中和更具体的功能推断；工具一致性与背景信息提供支持，但不等于底物已被实验证实。

:::

## 同一个 CAZyme，为什么几个工具不完全一致？ {#sec-target}
...
多个工具可能共享同源数据库和序列特征，因此三者一致不能被当成三次独立实验。两工具门槛定义的是本次主候选集合，不是所有项目通用的正确性证明。保留单工具分支有助于解释召回与特异性权衡，但不能在看到结果后混入主结论。

family、substrate 候选和 CGC 是不同证据层级。完整参考基因组上的 117 个 CGC 与宏基因组碎片上的结果不可直接按数量比较，因为上下文可见范围不同。若关键成员位于 contig 边缘，应降低完整簇的表述强度；序列预测本身仍不证明在当前样本条件下发生了相应代谢活动。

## 下载本例数据与分析代码 {#sec-reader-inputs}

[数据与代码清单](https://github.com/petemeng/metagenomics-best-practices/blob/e219a2cd01609cc208a89e291cbd363ed7f2fbd8/examples/37/files.tsv)列出了本例的输入表、分析脚本和环境文件（约 1.3 MiB）。本清单不含原始 FASTQ 或大型数据库；是否需要它们取决于下文的分析起点。清单中的已计算结果仅供核对；读取结果表不等于重新运行统计模型或上游工具。

在新建文件夹中运行下面的 R 代码，下载完成后即可按正文中的相对路径读取文件。需要核验文件版本时，可使用[带完整性检查的下载脚本](https://raw.githubusercontent.com/petemeng/metagenomics-best-practices/62d27927776d6fd61479767fc219d590fb8259d2/examples/37/download.R)。

```{r}
#| label: reader-input-download-37
#| eval: false
options(timeout = 600)
...
## 保留 CAZyme 注释方法与结论层级 {#sec-methods}
