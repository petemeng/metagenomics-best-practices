...
::: {.callout-tip title="能创建环境，还要确认命令实际从哪里运行"}

区分 Windows 管理命令与 Linux 分析命令；环境创建成功后，还要检查实际工具入口。

:::

## 在 Windows 上建立可检查的 Linux 分析环境 {#sec-target}

Windows 的管理命令与 Ubuntu 内的分析命令属于不同执行层。安装环境之前先确认当前终端，否则同名命令可能指向不同软件：
...
同名 Python 或 R 可以同时存在于 Windows、WSL、base 与项目环境中。排错时先确认当前终端、可执行路径和实际加载版本，再修改依赖；否则可能在一个环境安装包，却在另一个环境运行分析。把项目与高频读写放在合适的 Linux 文件系统，也比只增加线程更能避免不必要的 I/O 开销。

本例已验证的是原生 Linux 执行层，不能把它冒充某台 Windows 电脑的 WSL 验收。Windows 虚拟化、发行版版本和资源分配仍需在读者机器上确认。后续重建环境时应使用与目标平台匹配的锁文件，而不是跨操作系统直接复制安装目录。

## 下载本例数据与分析代码 {#sec-reader-inputs}

[数据与代码清单](https://github.com/petemeng/metagenomics-best-practices/blob/e219a2cd01609cc208a89e291cbd363ed7f2fbd8/examples/09/files.tsv)列出了本例的输入表、分析脚本和环境文件（约 4.4 MiB）。本清单不含原始 FASTQ 或大型数据库；是否需要它们取决于下文的分析起点。清单中的已计算结果仅供核对；读取结果表不等于重新运行统计模型或上游工具。

在新建文件夹中运行下面的 R 代码，下载完成后即可按正文中的相对路径读取文件。需要核验文件版本时，可使用[带完整性检查的下载脚本](https://raw.githubusercontent.com/petemeng/metagenomics-best-practices/62d27927776d6fd61479767fc219d590fb8259d2/examples/09/download.R)。

```{r}
#| label: reader-input-download-09
#| eval: false
options(timeout = 600)
...
## 在 Administrator PowerShell 安装并确认 WSL2 {#sec-code}
...
### WSL2 是 Linux 执行层，不是 Windows 版 Bash 主题 {#sec-theory}
