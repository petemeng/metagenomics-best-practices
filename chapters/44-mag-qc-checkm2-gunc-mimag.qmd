---
author: "Peter"
title: "MAG 质控：CheckM2、GUNC、完整 MIMAG 与 assembly graph"
...
::: {.callout-tip title="完整度与污染估计不能定位每一条错误 contig"}

CheckM2、GUNC 与 rRNA/tRNA 证据回答不同问题；完整 MIMAG 分级不能简化成完整度和污染两个阈值。

:::

## 完整度超过 90%，为什么还不一定是高质量 MAG？ {#sec-target}
...
### 完整度与污染估计不能定位每一条错误 contig

质量模型给出基因组层面的估计，可能遗漏局部错分、结构错误或近缘混合；高完整度也不保证所有必需 RNA 元件都被回收。本例 23 个 MAG 中只有 4 个达到完整 MIMAG HQ，说明二维质量散点的绿色区域不能直接充当最终等级。

GUNC 关注谱系不一致，rRNA/tRNA 检查关注明确结构需求，组装图与回比关注连接和局部支持，几者应互补。自动异常标记适合引导检查，不应直接触发删除。删改后要重新计算质量并保留版本，否则报告可能指向已经不存在的序列集合。

## 下载本例数据与分析代码 {#sec-reader-inputs}

[数据与代码清单](https://github.com/petemeng/metagenomics-best-practices/blob/e219a2cd01609cc208a89e291cbd363ed7f2fbd8/examples/44/files.tsv)列出了本例的输入表、分析脚本和环境文件（约 17.2 MiB）。本清单不含原始 FASTQ 或大型数据库；是否需要它们取决于下文的分析起点。清单中的已计算结果仅供核对；读取结果表不等于重新运行统计模型或上游工具。

在新建文件夹中运行下面的 R 代码，下载完成后即可按正文中的相对路径读取文件。需要核验文件版本时，可使用[带完整性检查的下载脚本](https://raw.githubusercontent.com/petemeng/metagenomics-best-practices/62d27927776d6fd61479767fc219d590fb8259d2/examples/44/download.R)。

```{r}
#| label: reader-input-download-44
#| eval: false
options(timeout = 600)
...
### CheckM2：标记与机器学习意义上的 completeness/contamination {#sec-theory}
...
### 从简化 >90/<5 升级为完整 MIMAG {#sec-audit}
