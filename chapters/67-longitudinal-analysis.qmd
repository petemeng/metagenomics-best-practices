...
::: {.callout-tip title="个体内变化与个体间差异需要分别估计"}

以受试者而非访视数计算独立性；模型和 bootstrap 都要保留主体结构与实际时间间隔。

:::

## 重复测了很多次，独立样本到底有多少？ {#sec-target}
...
![Lloyd-Price et al. 2019 Figure 3。原图用于定位纵向稳定性、群落转换和 *P. copri* 轨迹](../figures/67-lloyd-price-fig3-original.png){#fig-67-anchor}
...
某个变量在不同受试者之间与菌群有关，不代表同一个人该变量变化时菌群也随之变化。若研究目标是个体内关联，可将随时间变化的协变量拆为个人均值与偏离个人均值的部分；只放一个原始值，可能混合两种效应。随机截距处理相关性，但不会自动完成这种分解。

随机时间斜率允许轨迹不同，也仍需检查模型能否支持相应复杂度。随访更密集或失访与病情相关时，观察到的轨迹可能不是全队列的无偏变化。本例按人处理不确定性，独立单位仍是 107 名受试者，而非 1,523 次测量；反复检出也不能单独证明永久定植。

## 下载本例数据与分析代码 {#sec-reader-inputs}

[数据与代码清单](https://github.com/petemeng/metagenomics-best-practices/blob/e219a2cd01609cc208a89e291cbd363ed7f2fbd8/examples/67/files.tsv)列出了本例的输入表、分析脚本和环境文件（约 2.9 MiB）。本清单不含原始 FASTQ 或大型数据库；是否需要它们取决于下文的分析起点。清单中的已计算结果仅供核对；读取结果表不等于重新运行统计模型或上游工具。

在新建文件夹中运行下面的 R 代码，下载完成后即可按正文中的相对路径读取文件。需要核验文件版本时，可使用[带完整性检查的下载脚本](https://raw.githubusercontent.com/petemeng/metagenomics-best-practices/62d27927776d6fd61479767fc219d590fb8259d2/examples/67/download.R)。

```{r}
#| label: reader-input-download-67
#| eval: false
options(timeout = 600)
...
## 确定性选择一个 biospecimen 的 primary profile {#sec-code}

### 观测单位、实验单位与不确定性单位不是一回事 {#sec-theory}
...
## 构造可解释的 dysbiosis score

### “群落转换”必须先定义 estimand，再选择阈值
...
## 物种层 screen 使用 CLR、随机截距与两层 FDR

### 随机截距回答“每个人基线不同”，随机斜率回答“每个人变化速度不同”
...
### 定植稳定性至少要区分 abundance、detection 与 strain identity
...
同一受试者的 20 次采样不是 20 个独立样本。用 HMP2 真实纵向队列，把技术重复、不规则间隔、随机斜率、主体级 bootstrap 与‘检出不等于定植’一次讲清。
