...
::: {.callout-tip title="基因未检出，何时才能解释为真正缺失"}

参考集合和检出规则共同定义附属基因；未检出与真实缺失要区分，基因存在仍不等于表达。

:::

## 属于同一物种，为什么功能基因仍然不同？ {#sec-target}
...
[数据与代码清单](https://github.com/petemeng/metagenomics-best-practices/blob/e219a2cd01609cc208a89e291cbd363ed7f2fbd8/examples/52/files.tsv)列出了本例的输入表、分析脚本和环境文件（约 3.0 MiB）。本清单不含原始 FASTQ 或大型数据库；是否需要它们取决于下文的分析起点。清单中的已计算结果仅供核对；读取结果表不等于重新运行统计模型或上游工具。

在新建文件夹中运行下面的 R 代码，下载完成后即可按正文中的相对路径读取文件。需要核验文件版本时，可使用[带完整性检查的下载脚本](https://raw.githubusercontent.com/petemeng/metagenomics-best-practices/62d27927776d6fd61479767fc219d590fb8259d2/examples/52/download.R)。

```{r}
#| label: reader-input-download-52
#| eval: false
options(timeout = 600)
...
## 下载、校验并登记真实输入 {#sec-code}

### 物种 abundance 与菌株 gene repertoire 是两个问题 {#sec-theory}
