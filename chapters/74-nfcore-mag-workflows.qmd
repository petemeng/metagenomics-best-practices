---
title: "用 nf-core/mag 等 Nextflow / Snakemake 流程复现全上游"
...
::: {.callout-tip title="缓存命中与科学结果正确是两种验收"}

固定流程版本、参数、容器和数据库；仅通过 stub 测试不能证明真实数据分析成功。

:::

## 流程跑完了，哪些信息才让别人能够重跑？ {#sec-target}
...
4/5 个 task 被缓存说明工作流识别并复用了相应计算，不说明输入的生物学身份、数据库适用性或最终 MAG 质量正确。stub 测试验证调度与文件接口，也不会执行真实科学算法。应把控制流程通过、真实任务完成和科学结果可接受分别报告，避免一个绿色状态替代三种判断。

流程 release 固定后，数据库、容器和参数仍可能改变结果；仅记录流程版本不足以重建分析。恢复执行时还应确认外部输入与参考资源没有发生未记录的替换。相同软件产生相同结果是可重复性的一层，结论能否推广到其他队列仍需要独立验证。

## 下载本例数据与分析代码 {#sec-reader-inputs}

[数据与代码清单](https://github.com/petemeng/metagenomics-best-practices/blob/e219a2cd01609cc208a89e291cbd363ed7f2fbd8/examples/74/files.tsv)列出了本例的输入表、分析脚本和环境文件（约 0.6 MiB）。本清单不含原始 FASTQ 或大型数据库；是否需要它们取决于下文的分析起点。清单中的已计算结果仅供核对；读取结果表不等于重新运行统计模型或上游工具。

在新建文件夹中运行下面的 R 代码，下载完成后即可按正文中的相对路径读取文件。需要核验文件版本时，可使用[带完整性检查的下载脚本](https://raw.githubusercontent.com/petemeng/metagenomics-best-practices/62d27927776d6fd61479767fc219d590fb8259d2/examples/74/download.R)。

```{r}
#| label: reader-input-download-74
#| eval: false
options(timeout = 600)
...
## 下载并核验 workflow source {#sec-code}
...
### 先忠实复现 2022 pipeline，再升级到 2026 contract {#sec-audit}
...
## 先做 schema/DAG stub，再做真实 pilot

### `group`、co-assembly 与 coverage mapping 是三件事
...
### pipeline success、stub success 与 scientific acceptance 分开
...
## 用 nf-core/funcscan 组织 ARG、AMP、BGC 与 CAZyme
...
## Snakemake 接管自定义结果核对，而不是重复调度上游

### Nextflow 与 Snakemake 锁的是同一组科学对象
...
一键运行不是可复现。真正要锁住的是 pipeline tag 与 commit、Nextflow、参数、Apptainer 镜像、数据库 checksum、输入单位和 trace；stub-run 只能证明 DAG，不能证明 MAG 质量。
