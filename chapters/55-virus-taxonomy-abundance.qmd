---
author: "Peter"
title: "病毒分类与丰度：vOTU、breadth、taxonomy 与 lifestyle"
...
::: {.callout-tip title="分类标签和丰度表并不描述同一件事"}

目录覆盖、未检出和定量分母都会影响比较；相对丰度或分类标签不能证明生活史或活性。

:::

## 病毒检出与相对丰度，分别有哪些限制？ {#sec-target}
...
## 解析 supplement，校验来源与 accession {#sec-code}
...
### 先定义计量单位：sequence、vOTU 还是 taxon {#sec-theory}
...
本数据采用论文的 `minimum identity = 90%`，再以 $\bar d\ge1$ 且 $B_{1\times}\ge75\%$ 定义 presence。`samtools coverage` 的 `coverage` 是 depth ≥1 的位置比例，`meandepth` 才是平均深度 [@li2009samtools]；两列不能拿错。
...
### 论文内部有 75% 与 70% 两种 breadth 表述 {#sec-audit}
