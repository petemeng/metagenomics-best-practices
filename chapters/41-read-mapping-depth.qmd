---
author: "Peter"
title: "回比与深度：reads 回比与 JGI 深度矩阵"
...
::: {.callout-tip title="相同平均深度，为什么覆盖证据可能完全不同"}

把 depth 与 breadth 分开看；多重比对和配对规则会影响深度矩阵，不能只靠均值筛选。

:::

## 平均深度相近，覆盖范围也相同吗？ {#sec-target}
...
### 相同平均深度，为什么覆盖证据可能完全不同？

平均值可以由少数高覆盖区域拉高；breadth 则说明有多少参考位置得到支持。多重比对或保守区域富集时，高 depth 与低 breadth 可以同时出现。因此检出、丰度和分箱输入要分别说明各自使用的覆盖统计，不能只保留一个均值。

深度矩阵还必须使用同一个 contig 坐标与样本顺序。两个样本几乎所有 contigs 都有正深度，不意味着它们具有足够不同的共丰度轨迹来区分基因组。样本间信息相近时，分箱仍需要其他证据；增加矩阵列名并不等于增加独立的覆盖变化。

## 下载本例数据与分析代码 {#sec-reader-inputs}

[数据与代码清单](https://github.com/petemeng/metagenomics-best-practices/blob/e219a2cd01609cc208a89e291cbd363ed7f2fbd8/examples/41/files.tsv)列出了本例的输入表、分析脚本和环境文件（约 4.1 MiB）。本清单不含原始 FASTQ 或大型数据库；是否需要它们取决于下文的分析起点。清单中的已计算结果仅供核对；读取结果表不等于重新运行统计模型或上游工具。

在新建文件夹中运行下面的 R 代码，下载完成后即可按正文中的相对路径读取文件。需要核验文件版本时，可使用[带完整性检查的下载脚本](https://raw.githubusercontent.com/petemeng/metagenomics-best-practices/62d27927776d6fd61479767fc219d590fb8259d2/examples/41/download.R)。

```{r}
#| label: reader-input-download-41
#| eval: false
options(timeout = 600)
...
## 准备 checksum-locked 输入 {#sec-code}

### “比对上”至少有四层分母 {#sec-theory}
...
### 从一个 mapping rate 升级为分层 ledger {#sec-audit}
