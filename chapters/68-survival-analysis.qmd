---
title: "微生物组与生存结局：Cox / Kaplan–Meier / 时间依赖 ROC"
...
::: {.callout-tip title="风险比与临床预测性能为何不能互换"}

优先报告连续效应；cutoff 只在训练集确定，并检查比例风险和时间依赖性能。

:::

## 生存曲线好看，是否来自事后挑选 cutoff？ {#sec-target}
...
![Spencer et al. 2021 Figure 3A。原图用于定位 PFS、删失标记、风险表与 log-rank 检验](../figures/68-spencer-fig3a-original.png){#fig-68-anchor}
...
校正后风险比 0.933 的区间跨 1，不能说明真实效应为零，也不能把未校正显著结果当作独立预后证据。Cox 风险比描述给定协变量下的瞬时风险关系，不是固定时间点的风险差或治愈概率。比例风险不成立时，一个常数风险比还可能遮住随时间变化的关联。

模型能否预测，需要在预先指定时间点检查区分度、校准和不确定性；它不由某个系数的 P 值决定。110 人中仅 61 个事件，33 人测试集的性能尤其需要谨慎解释。删失机制、时间起点和训练阶段的特征选择，都比事后寻找更好看的 cutoff 更能决定结论是否可靠。

## 下载本例数据与分析代码 {#sec-reader-inputs}

[数据与代码清单](https://github.com/petemeng/metagenomics-best-practices/blob/e219a2cd01609cc208a89e291cbd363ed7f2fbd8/examples/68/files.tsv)列出了本例的输入表、分析脚本和环境文件（约 1.2 MiB）。本清单不含原始 FASTQ 或大型数据库；是否需要它们取决于下文的分析起点。清单中的已计算结果仅供核对；读取结果表不等于重新运行统计模型或上游工具。

在新建文件夹中运行下面的 R 代码，下载完成后即可按正文中的相对路径读取文件。需要核验文件版本时，可使用[带完整性检查的下载脚本](https://raw.githubusercontent.com/petemeng/metagenomics-best-practices/62d27927776d6fd61479767fc219d590fb8259d2/examples/68/download.R)。

```{r}
#| label: reader-input-download-68
#| eval: false
options(timeout = 600)
...
## 从公开谱表确定性汇总预设属 {#sec-code}

### 删失意味着“尚未观察到”，不是“没有事件” {#sec-theory}
...
## 交叉拟合时间依赖风险，而不是只报 apparent AUC

### Cox 回答相对风险，不直接给“多活几个月”
...
生存曲线应同时标出删失状态和风险集人数。若期刊要求黑白打印，`High/Low` 除颜色外还应增加 line type，避免灰阶下无法辨认。

## 回到最初的问题

KM 曲线最容易画，也最容易被 cutoff 数据泄漏做得过分漂亮。用真实 shotgun + PFS 数据，把连续 Cox、比例风险、训练/测试隔离和时间依赖 ROC 串成一条可投稿的证据链。
