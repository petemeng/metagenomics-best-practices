---
title: "MetaPhlAn4：marker 基因、SGB 与物种级组成分析"
author: "Peter"
date: last-modified
draft: false
format:
...
::: {.callout-tip title="零未分类比例不等于所有 reads 都已找到物种"}

丰度来自 marker coverage，不是直接数比对命中；SGB 名称、未分类部分和 marker 支持要一起解释。

:::

## MetaPhlAn 的物种丰度，是怎样从 marker 得到的？ {#sec-target}
...
[数据与代码清单](https://github.com/petemeng/metagenomics-best-practices/blob/e219a2cd01609cc208a89e291cbd363ed7f2fbd8/examples/15/files.tsv)列出了本例的输入表、分析脚本和环境文件（约 0.3 MiB）。本清单不含原始 FASTQ 或大型数据库；是否需要它们取决于下文的分析起点。清单中的已计算结果仅供核对；读取结果表不等于重新运行统计模型或上游工具。

在新建文件夹中运行下面的 R 代码，下载完成后即可按正文中的相对路径读取文件。需要核验文件版本时，可使用[带完整性检查的下载脚本](https://raw.githubusercontent.com/petemeng/metagenomics-best-practices/62d27927776d6fd61479767fc219d590fb8259d2/examples/15/download.R)。

```{r}
#| label: reader-input-download-15
#| eval: false
options(timeout = 600)
...
### 为什么用 clade-specific markers，而不是整库所有序列 {#sec-theory}
