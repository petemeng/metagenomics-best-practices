...
::: {.callout-tip title="高丰度、经常检出与持续定植不是同一件事"}

先看每个样本的组成，再比较检出率；核心成员必须同时说明检测阈值、观察单位与稳定性。

:::

## 组成占比高，为什么还不能叫核心成员？ {#sec-target}
...
![Human Microbiome Project Consortium 2012 Figure 3。颜色强度编码 prevalence，圆点面积编码 non-zero abundance；原图按 CC BY-NC-SA 3.0 使用，](../figures/25-hmp-fig3-original.jpg){#fig-25-anchor}
...
HMP 图中的检出率和检出后丰度分别描述两个维度。少数人中非常高丰度的物种不一定是人群核心，很多人中低丰度检出的物种则可能满足 core 定义。这里先选 490 个 subject–habitat 代表，避免重复测量较多的人获得更大权重。

阈值附近的物种不应被一条硬线赋予永久身份。Wilson 区间、受试者 bootstrap 与阈值网格用于展示当前队列的不确定性；它们并不证明跨人群或跨时间稳定定植。不同 body habitat 的检测与组成背景也不同，宜分别解释，而不是合成一个无条件的“人体核心菌群”。

## 下载本例数据与分析代码 {#sec-reader-inputs}

[数据与代码清单](https://github.com/petemeng/metagenomics-best-practices/blob/e219a2cd01609cc208a89e291cbd363ed7f2fbd8/examples/25/files.tsv)列出了本例的输入表、分析脚本和环境文件（约 3.4 MiB）。本清单不含原始 FASTQ 或大型数据库；是否需要它们取决于下文的分析起点。清单中的已计算结果仅供核对；读取结果表不等于重新运行统计模型或上游工具。

在新建文件夹中运行下面的 R 代码，下载完成后即可按正文中的相对路径读取文件。需要核验文件版本时，可使用[带完整性检查的下载脚本](https://raw.githubusercontent.com/petemeng/metagenomics-best-practices/62d27927776d6fd61479767fc219d590fb8259d2/examples/25/download.R)。

```{r}
#| label: reader-input-download-25
#| eval: false
options(timeout = 600)
...
![主核心定义为 species abundance ≥0.01% 且 prevalence ≥80%。左侧展示全部跨 habitat 成员及每个 habitat 得分最高的 5 个成员；共有 92 个成员满足主核心定义。右侧给出 112 个 detection × prevalence 定义分支。](../figures/25-core-membership-sensitivity.png){#fig-25-core}
