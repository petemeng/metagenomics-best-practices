...
## RNA/DNA 比值高，是否代表表达更活跃？ {#sec-target}
...
![Lloyd-Price et al. 2019 Figure 3。原图用于定位纵向多组学问题；文章采用 CC BY 4.0](../figures/64-lloyd-price-fig3-original.png){#fig-64-anchor}
...
RNA 与 DNA 表分别按各自总量归一化。某条通路的 RNA/DNA 指数升高，既可能来自该通路相对转录份额变化，也可能来自两层其他成员或总量的变化。低 DNA 分母与 pseudocount 还会放大比值的不稳定性，因此应同时查看原始两层信号，而不是只按比值排序。

本例 173 条通路表现出相对转录分配差异，但疾病比较中 202 条可检验通路没有一条通过 BH 门槛。前者描述同一样本两层测量的关系，后者描述组间差异，两者不矛盾。“相对 RNA 富集”也不等于疾病特异性激活，更不等于单位细胞的绝对调控变化。

## 下载本例数据与分析代码 {#sec-reader-inputs}

[数据与代码清单](https://github.com/petemeng/metagenomics-best-practices/blob/e219a2cd01609cc208a89e291cbd363ed7f2fbd8/examples/64/files.tsv)列出了本例的输入表、分析脚本和环境文件（约 2.5 MiB）。本清单不含原始 FASTQ 或大型数据库；是否需要它们取决于下文的分析起点。清单中的已计算结果仅供核对；读取结果表不等于重新运行统计模型或上游工具。

在新建文件夹中运行下面的 R 代码，下载完成后即可按正文中的相对路径读取文件。需要核验文件版本时，可使用[带完整性检查的下载脚本](https://raw.githubusercontent.com/petemeng/metagenomics-best-practices/62d27927776d6fd61479767fc219d590fb8259d2/examples/64/download.R)。

```{r}
#| label: reader-input-download-64
#| eval: false
options(timeout = 600)
...
::: {.callout-tip title="两个相对量相除，为什么还不是每细胞表达"}
...
## 只取 unstratified pathways，并严格配对样本 {#sec-code}
...
## co-detection gate 与相对 RNA/DNA 指数

### DNA 是功能潜力，RNA 是转录本份额 {#sec-theory}
