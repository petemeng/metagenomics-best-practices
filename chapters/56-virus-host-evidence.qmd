---
author: "Peter"
title: "病毒—宿主关联：证据等级、iPHoP 与假阳性审计"
...
::: {.callout-tip title="超出适用范围的预测为什么容易显得可信"}

不同证据不能压成一个无条件的宿主标签；预测结果应保留冲突、不确定性与尚未验证的范围。

:::

## 一条病毒—宿主预测，距离证实还有多远？ {#sec-target}
...
本例把不属于目标适用域的序列作为负对照，仍出现 1,018 次宿主调用，占 8,128 条序列的 12.5%。这说明“软件返回了标签”与“问题在该模型的适用范围内”是两件事。内部得分只能在其定义和评估范围内解读，不能自动变成任意新样本上的感染概率。

若多种证据共享参考库、相似性基础或训练来源，它们的一致也不是完全独立的支持。应允许未判定，并明确模型适用域、缺失证据和候选解释的不确定性。序列层的关联判断不能代替实际感染证据，也不足以推出宿主范围或传播能力。

## 重建论文级 aggregate evidence {#sec-code}

### 最高等级不是“同一 MAG”，而是可检查的整合边界 {#sec-theory}
...
### 把“作者使用的工具”升级为“今天可检查的边” {#sec-audit}
