---
title: "功能谱归一化与解读：CPM、relative abundance、coverage 与特殊行"
author: "Peter"
date: last-modified
draft: false
format:
...
::: {.callout-tip title="删除特殊行后，100% 的含义已经改变"}

归一化前先决定特殊行是否保留；删除未解释部分后重新闭合，不能再把 100% 解释为功能覆盖率。

:::

## 换一个分母，功能图会怎样变化？ {#sec-target}
...
本例去掉特殊行后，pathway 分母从约 102,782 降到 3,568。重新闭合所得比例描述的是可解释普通通路内部的组成，而不是全部输入的功能覆盖；两种图可以都合理，但必须写清条件分母。CPM 与相对丰度只是同一次闭合的不同数值刻度，不构成两个独立归一化证据。

特殊行也不能都解释为“未知物种”：未映射、未归组与未整合反映不同分析阶段。应保留它们各自的比例，再决定统计目标是否条件于已注释部分。pathway coverage 则保留原生范围，不应当作 abundance 继续闭合或稀释。

## 下载本例数据与分析代码 {#sec-reader-inputs}

[数据与代码清单](https://github.com/petemeng/metagenomics-best-practices/blob/e219a2cd01609cc208a89e291cbd363ed7f2fbd8/examples/20/files.tsv)列出了本例的输入表、分析脚本和环境文件（约 8.7 MiB）。本清单不含原始 FASTQ 或大型数据库；是否需要它们取决于下文的分析起点。清单中的已计算结果仅供核对；读取结果表不等于重新运行统计模型或上游工具。

在新建文件夹中运行下面的 R 代码，下载完成后即可按正文中的相对路径读取文件。需要核验文件版本时，可使用[带完整性检查的下载脚本](https://raw.githubusercontent.com/petemeng/metagenomics-best-practices/62d27927776d6fd61479767fc219d590fb8259d2/examples/20/download.R)。

```{r}
#| label: reader-input-download-20
#| eval: false
options(timeout = 600)
...
### 保存 native table，并把每个派生表写进文件名 {#sec-audit}
...
### CPM 与 relative abundance 的信息相同，刻度不同 {#sec-theory}
