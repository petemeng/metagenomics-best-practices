---
author: "Peter"
title: "新物种与系统基因组：novel MAG、GToTree 与 IQ-TREE"
...
::: {.callout-tip title="高分支支持不等于物种界限已被证明"}

把基因组质量、相似性边界与系统发育支持合起来判断；树上独立成枝不自动等于新物种。

:::

## 一条新分支，是否足以支持新物种？ {#sec-target}
...
bootstrap 支持评估的是给定比对与模型下的分支稳定性，不覆盖参考选择、基因缺失、污染和基因间冲突的全部不确定性。某个 MAG 单独成枝可能只是缺少近缘参考；新物种主张还需要合适的相似性范围及基因组质量证据。

本例一个 MAG 因 marker recovery 不足没有入树，但它仍有支持已有物种归属的 ANI/AF 信息。不同证据失败应分别解释，不能把“未入树”当作“新颖”。当前数据支持零个 novel species 是有效结果，不需要通过改变图形或参考集合制造新分支故事。

## 下载本例数据与分析代码 {#sec-reader-inputs}

[数据与代码清单](https://github.com/petemeng/metagenomics-best-practices/blob/e219a2cd01609cc208a89e291cbd363ed7f2fbd8/examples/47/files.tsv)列出了本例的输入表、分析脚本和环境文件（约 0.9 MiB）。本清单不含原始 FASTQ 或大型数据库；是否需要它们取决于下文的分析起点。清单中的已计算结果仅供核对；读取结果表不等于重新运行统计模型或上游工具。

在新建文件夹中运行下面的 R 代码，下载完成后即可按正文中的相对路径读取文件。需要核验文件版本时，可使用[带完整性检查的下载脚本](https://raw.githubusercontent.com/petemeng/metagenomics-best-practices/62d27927776d6fd61479767fc219d590fb8259d2/examples/47/download.R)。

```{r}
#| label: reader-input-download-47
#| eval: false
options(timeout = 600)
...
## 重建输入与 novelty pre-tree audit {#sec-code}

### 先问“是否已有 species assignment” {#sec-theory}
