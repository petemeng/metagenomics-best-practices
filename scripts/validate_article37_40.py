...
def audit_chapter(article: int, chapter: Path, figures: list[str], checks: Checks) -> list[dict[str, str]]:
    ...
    expected = {
        ...
        "takeaways": "## 先确定这一点" in text,
        "theory": "#sec-theory" in text, "setup": "#sec-setup" in text,
        "code": "#sec-code" in text, "audit": "#sec-audit" in text,
        "publication": "## 图中应该保留哪些信息" in text, "pitfalls": ".callout-caution" in text,
        "methods": "## #sec-methods" in text, "own-data": "## #sec-own-data" in text,
        "references": "## 参考文献" in text, "resource-contract": all(x in text for x in ("RAM", "磁盘", "CPU", "耗时")),
