...
[在线阅读完整教程](https://petemeng.github.io/metagenomics-best-practices/)

## 从一篇教程开始

技术章节开头提供“下载本例数据与分析代码”：在新文件夹运行该处代码，
即可取得本章使用的公开表格、分析脚本与环境说明。64 份逐章
[文件清单](examples/)区分输入数据和已计算结果；下载结果表用于核对，
不等于重新拟合模型。原始 FASTQ 和大型数据库不包含在这些小文件清单中。

每章把代码与对应图表放在一起，随结果说明方法选择、观察单位和结论边界。
导读与证据比较章节直接展示案例和判断，不要求安装分析环境。
微信公众号保留问题、数据入口、关键代码和结果解释；完整运行说明见章节源码。
...
- 公众号发布包先在本地完成严格检查；经明确授权可更新对应草稿，
  新稿回读核对通过后才删除映射的旧稿，不动其他草稿，不自动发布或群发。
- 线上网站部署已验证的静态产物到 `gh-pages`；传统源码 CI 的 deploy job
  仍默认关闭。网站的 `version.json` 记录部署对应的源代码版本。
...
- 全系列进一步说明输入单位、方法假设、比较分母、独立验证和结论边界；
  保留公开数据入口与实际结果，不把注释、模型输出或软件成功状态等同于生物学验证。
